...
//!
//! Reads plaintext config and plaintext secrets from separate TOML files,
...
pub struct Env {
    ...
    #[clap(long)]
    pub config: PathBuf,
    /// Path to plaintext TOML secrets file
    #[clap(long)]
    pub secrets: PathBuf,
}
...
struct OrderTakerConfig {
    ...
    /// Maximum position size per symbol (in shares). Must be positive.
    max_position_per_symbol: Positive<FractionalShares>,
    /// Maximum total capital at risk (in USDC).
    max_total_exposure_usdc: Usdc,
...
struct HoldPoolConfig {
    ...
    /// Maximum value to hold in the pool across all symbols (USDC
    /// equivalent).
    max_pool_value_usdc: Usdc,
    /// Per-symbol cap (in shares). Must be positive.
    max_per_symbol: Positive<FractionalShares>,
}
...
pub(crate) struct OrderTakerCtx {
    ...
    pub(crate) max_position_per_symbol: Positive<FractionalShares>,
    pub(crate) max_total_exposure_usdc: Usdc,
...
pub(crate) struct HoldPoolCtx {
    ...
    pub(crate) max_pool_value_usdc: Usdc,
    pub(crate) max_per_symbol: Positive<FractionalShares>,
}
...
"#;
    ...
    #[test]
    fn negative_max_position_per_symbol_rejected() {
        let config = valid_config_toml().replace(
            "max_position_per_symbol = 100.0",
    ...
    #[test]
    fn negative_max_per_symbol_rejected() {
        let config = valid_config_toml().replace("max_per_symbol = 50.0", "max_per_symbol = -1.0");
