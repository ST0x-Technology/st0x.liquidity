...
##### Client-to-Server: Inquiries

The client sends inquiries to request data or (in the future) adjustments:

```rust
enum Inquiry {
    /// "Bring me up to speed from this point"
    Sync { last_sequence: Option<u64> },
    /// "I need more data" (e.g., paginate trade history)
    Request(DataRequest),
...
5. **System Status**: Connection health, circuit breaker state, auth status.
   Populated via WebSocket statements.
...
Bidirectional channel using the Statement/Inquiry protocol. Server sends
`Envelope` messages (statements and deltas); client sends `Inquiry` messages
(sync requests, data requests). See WebSocket Communication Protocol section
above.
