...
channel" below). The `[alerts]` config section is **required** and configures
the gas monitor: operational configuration must be explicit, and a bot without a
gas monitor silently stops transacting when its wallet runs dry, so an absent
section fails startup rather than skipping the monitor.
...
- When the balance drops **below** that chain's explicit threshold (its entry in
  the `[alerts.low_balance_thresholds]` table, a decimal-ETH amount parsed to
  wei at startup; a missing, malformed, or zero value fails fast with no
  cross-chain fallback), it raises an alert: a structured `error!` log (target
  `gas`) carrying the wallet, chain, current balance and threshold, plus an
  operational-alert notification (see "Structured log channel" below).
