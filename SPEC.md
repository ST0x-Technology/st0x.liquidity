...
`MintAuth(token, recipient, amount,
nonce)` signature. The recipient of
rebalancing mints is this bot, so it must sign and deliver that authorization.
The point is a two-key security property: the mint-signing key (issuance,
Turnkey `S01Minter`) and the recipient-authorization key (this bot's wallet, a
different Turnkey wallet) are distinct — neither key alone can both mint and
authorize.
...
- **Mode discovery**: per-asset `vault_mode` (`"vault_direct"` |
  ...
  source of truth; this bot keeps no asset-mode list. Vault-direct assets take
  the pre-existing path completely unchanged.
...
- **Configuration**: `[orchestrator] address = "0x…"` in the plaintext config,
  ...
  without it fails loudly rather than guessing an address, and a failed
  vault-mode read fails closed (an indeterminate mode is never treated as
  vault-direct).
...
- `RecordMintAuthorizationDelivered` requires a signed authorization
  (acknowledging an unsigned mint is a caller bug). It normally executes AFTER
  the mint advanced past MintAccepted: `Poll` holds the per-aggregate lock for
  the whole Alpaca wait, and issuance completes the mint only once the
  authorization arrived, so the acknowledgement queues behind `Poll` and lands
  on `TokensReceived` or later — recorded there as an audit fact with no state
  change. No-op on already-delivered and on terminal states.
