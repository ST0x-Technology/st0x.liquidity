...
operator alert, and records a structured circuit transition with the worker
name, error, failure count, and cooldown. The process and sibling workers remain
live. After a five-minute cooldown, the same worker resumes without a process
restart and records a structured recovery transition. A successful job resets
the consecutive-failure count. Best-effort workers retain and log terminal jobs
without pausing.
