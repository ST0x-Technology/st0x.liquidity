...
burn (including a reburn after a burn reverted, on resume or in process after a
confirm-time revert; the reverted hash stays recorded during the check so a
restart there still reburns) or a BaseToAlpaca deposit send, the bot reads the
wallet's USDC balance and compares it with the outstanding total. The sending
...
between `BeginBridging` and the burn. A shortfall pages the operator
