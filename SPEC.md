...
Turnkey's AWS Nitro secure enclaves for low-latency signing (`turnkey` feature).
A local-signer implementation is available for development and testing
(`local-signer` feature). Domain logic is decoupled from the signing backend.
See [crates/evm/](crates/evm/) for implementation details.
