...
pub(crate) fn counter_trading_session_for_iso(iso: &str) -> &'static str {
    if market_session_for_iso(iso) == MarketSession::Rth {
...
pub(crate) fn matches_market_session_filter(
    ...
    match filter.unwrap_or(PnlMarketSessionFilter::All) {
        ...
        PnlMarketSessionFilter::Pre => market_session_for_iso(iso) == MarketSession::Pre,
        PnlMarketSessionFilter::Rth => market_session_for_iso(iso) == MarketSession::Rth,
        PnlMarketSessionFilter::Post => market_session_for_iso(iso) == MarketSession::Post,
        PnlMarketSessionFilter::Overnight => {
            market_session_for_iso(iso) == MarketSession::Overnight
        }
        PnlMarketSessionFilter::Weekend => market_session_for_iso(iso) == MarketSession::Weekend,
    }
}
