...
pub(crate) fn settings_from_ctx(ctx: &st0x_config::Ctx) -> st0x_dto::Settings {
    ...
    let assets = ctx
        ...
        .map(|(symbol, config)| {
            ...
            // rejects either session flag being enabled for a symbol that
            // trades on no chain, so a disabled asset never carries a live
            ...
            let counter_trading = match config.trading {
                OperationMode::Enabled => st0x_dto::CounterTrading::Enabled {
                    ...
                    overnight: ctx.assets.is_overnight_enabled(symbol),
                },
...
mod tests {
    ...
    use st0x_execution::{
        ClientOrderId, ExecutorOrderId, FractionalShares, Positive, SupportedExecutor, Symbol, Usd,
    };
