    ...
    Convert: Fn(&Entity, &Entity::Id) -> TransferOperation,
{
    let ids = match load_all_ids::<Entity>(pool).await {
        ...
        Err(error) => {
            ...
            warnings.push(Warning::CategoryUnavailable { category });
            return;
        }
    };

    for id in &ids {
        let Some(entity) = try_load_entity::<Entity>(pool, id, &category, warnings).await else {
            continue;
        };

        classify(convert(&entity, id), cutoff, active, recent);
    }
}

async fn try_load_entity<Entity>(
    pool: &SqlitePool,
    id: &Entity::Id,
    category: &TransferCategory,
    warnings: &mut Vec<Warning>,
    ...
    <Entity::Id as FromStr>::Err: Debug,
{
    match load_entity::<Entity>(pool, id).await {
        ...
        Ok(None) => {
            ...
            record_replay_failure(id, category, warnings);
            None
        }
        Err(error) => {
...
fn record_replay_failure(
    ...
    warnings.push(Warning::AggregateReplayFailed {
