//! Mock implementation of CrossVenueTransfer for USDC, used in testing.
...
/// Parameters captured from a hedging-to-market-making (alpaca to base) call.
#[derive(Debug, Clone)]
pub(crate) struct AlpacaToBaseCall {
    pub(crate) amount: Usdc,
}

/// Parameters captured from a market-making-to-hedging (base to alpaca) call.
#[derive(Debug, Clone)]
pub(crate) struct BaseToAlpacaCall {
    pub(crate) amount: Usdc,
}

/// Mock USDC cross-venue transfer for testing.
///
...
impl MockUsdcRebalance {
    ...
    /// Creates a mock that fails hedging-to-market-making calls.
    ...
    /// Creates a mock that fails market-making-to-hedging calls.
...
/// withdraw, bridge, deposit to vault.
#[async_trait]
impl CrossVenueTransfer<HedgingVenue, MarketMakingVenue> for MockUsdcRebalance {
    type Asset = Usdc;
    ...
    async fn transfer(&self, asset: Self::Asset) -> Result<(), Self::Error> {
        ...
        *self.last_alpaca_to_base.lock().unwrap() = Some(AlpacaToBaseCall { amount: asset });
...
/// Base -> Alpaca (market-making -> hedging): withdraw from vault, bridge,
/// deposit USDC, convert to USD.
#[async_trait]
impl CrossVenueTransfer<MarketMakingVenue, HedgingVenue> for MockUsdcRebalance {
    type Asset = Usdc;
    type Error = UsdcTransferError;
    ...
    async fn transfer(&self, asset: Self::Asset) -> Result<(), Self::Error> {
        ...
        *self.last_base_to_alpaca.lock().unwrap() = Some(BaseToAlpacaCall { amount: asset });
