...
impl<Chain: Wallet> CrossVenueCashTransfer<Chain> {
    ...
    /// Drives the transfer from `Bridging`/`Attested` through to terminal.
    /// Re-polls the Circle attestation (idempotent) so we obtain a fresh
    /// [`AttestationResponse`] suitable for [`Bridge::mint`].
