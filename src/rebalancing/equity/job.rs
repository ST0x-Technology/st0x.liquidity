...
//!
//! The per-symbol `equity_in_progress` guard is normally cleared event-driven
//! when the aggregate reaches a terminal state (success or a recorded failure).
//! If every worker attempt fails before an aggregate is persisted, the terminal
//! attempt hook removes only the exact guard owned by that job. A transient
//! failure that schedules another retry keeps the guard latched so automation
//! does not re-arm a fresh transfer on top of a partial one.
...
impl Job<TransferEquityToMarketMakingCtx> for TransferEquityToMarketMaking {
    ...
    async fn on_terminal_attempt(
        ...
        warn!(
            ...
            %task_identity,
            released,
            "Terminal mint attempt checked its exact ownership claim"
        );
        Ok(())
    }
}
...
pub(crate) struct TransferEquityToHedging {
    ...
    pub(crate) symbol: Symbol,
    pub(crate) quantity: FractionalShares,
    /// Exact ownership claim held by this job. Missing legacy payload fields
...
impl Job<TransferEquityToHedgingCtx> for TransferEquityToHedging {
    ...
    async fn on_terminal_attempt(
        ...
        warn!(
            ...
            %task_identity,
            released,
            "Terminal redemption attempt checked its exact ownership claim"
        );
        Ok(())
    }
}
...
mod tests {
    ...
    async fn submit_requested_mint(
        ...
        ctx.mint_store
            ...
            .await
            .expect("SubmitMintRequest must transition to MintAccepted");
    }
    ...
    /// each listing AAPL with the given `wrapped_equity_recovery` mode; Base
    /// is the primary chain.
    ...
    fn payload_roundtrips_through_json() {
        ...
        assert_eq!(
            ...
            "missing generation must default to the legacy sentinel"
        );
        assert_eq!(
    ...
    #[tokio::test]
    async fn terminal_mint_cleanup_releases_absent_exact_owner_idempotently() {
        for generation in [
            GuardGeneration::default(),
            GuardGeneration::from_parts(NonZeroU32::new(3).unwrap(), 1),
        ] {
            let symbol = Symbol::new("AAPL").unwrap();
            let ctx = test_ctx(Arc::new(RecordingResume::success())).await;
            ctx.equity_in_progress
                .write()
                .unwrap()
    ...
    #[tokio::test]
    async fn terminal_mint_cleanup_preserves_existing_lifecycle() {
        let symbol = Symbol::new("AAPL").unwrap();
        let generation = GuardGeneration::from_parts(NonZeroU32::new(3).unwrap(), 2);
        let issuer_request_id = issuer_request_id("terminal-mint-with-intent");
        let ctx = test_ctx(Arc::new(RecordingResume::success())).await;
        ctx.mint_store
            .send(
                &issuer_request_id,
        ...
        Job::on_terminal_attempt(
            &job,
            &ctx,
            &TaskIdentity::for_test("terminal-mint-with-intent"),
        )
        .await
        .unwrap();
    ...
    async fn terminal_mint_cleanup_preserves_legacy_and_newer_owners() {
        ...
        let legacy_job = TransferEquityToMarketMaking {
        ...
        Job::on_terminal_attempt(&legacy_job, &ctx, &task_identity)
            .await
            .unwrap();
        Job::on_terminal_attempt(&old_job, &ctx, &task_identity)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn terminal_redemption_cleanup_releases_absent_exact_owner() {
        let symbol = Symbol::new("AAPL").unwrap();
        let generation = GuardGeneration::from_parts(NonZeroU32::new(5).unwrap(), 1);
        let ctx = redemption_test_ctx(
            Arc::new(RecordingRedemptionResume {
                fail: false,
                captured: Mutex::new(None),
        ...
        Job::on_terminal_attempt(
            &job,
            &ctx,
            &TaskIdentity::for_test("terminal-redemption-cleanup"),
        )
        .await
        .unwrap();
    ...
    #[tokio::test]
    async fn terminal_redemption_cleanup_preserves_legacy_and_newer_owners() {
        let symbol = Symbol::new("AAPL").unwrap();
        let current_generation = GuardGeneration::from_parts(NonZeroU32::new(6).unwrap(), 2);
        let current_owner = GuardState::ActiveTransfer {
            generation: current_generation,
        };
        let ctx = redemption_test_ctx(
            Arc::new(RecordingRedemptionResume {
        ...
        for (owner, generation) in [
            (current_owner.clone(), GuardGeneration::default()),
            (
                current_owner,
            ...
            ctx.equity_in_progress
                .write()
                .unwrap()
                .insert(symbol.clone(), owner.clone());
            let job = TransferEquityToHedging {
            ...
            Job::on_terminal_attempt(
                &job,
                &ctx,
                &TaskIdentity::for_test("preserved-redemption-cleanup"),
            )
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn terminal_redemption_cleanup_preserves_existing_lifecycle() {
        let symbol = Symbol::new("AAPL").unwrap();
        let generation = GuardGeneration::from_parts(NonZeroU32::new(5).unwrap(), 2);
        let aggregate_id = redemption_aggregate_id("terminal-redemption-with-intent");
        let ctx = redemption_test_ctx(
            Arc::new(RecordingRedemptionResume {
                fail: false,
                captured: Mutex::new(None),
        ...
        Job::on_terminal_attempt(
            &job,
            &ctx,
            &TaskIdentity::for_test("terminal-redemption-with-intent"),
        )
        .await
        .unwrap();
