...
async fn equity_offchain_imbalance_triggers_mint() {
    ...
    let equity_transfer = build_equity_transfer(&pool, raindex, tokenizer, wrapper);
...
async fn equity_onchain_imbalance_triggers_redemption() {
    ...
    let wrapper: Arc<dyn crate::wrapper::Wrapper> =
        Arc::new(MockWrapper::new().with_unwrapped_token(token_address));
    let equity_transfer = build_equity_transfer(&pool, raindex, tokenizer, wrapper);
