...
async fn performance_reliability(
    ...
    let entries = match state.ctx.log_dir.clone() {
        Some(log_dir) => {
            let filter = LogFilter {
                search_lower: None,
            ...
            .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
            if total > MAX_RELIABILITY_LOG_ENTRIES {
                warn!(
                    total,
...
async fn performance_infra(
    ...
    let range = ReportRange { from, to };
    let monitor = load_monitor_telemetry(&state.pool, &range, state.ctx.evm.orderbook)
        .await
        .inspect_err(|error| error!(%error, "Failed to load monitor telemetry"))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let dependencies = load_dependency_stats(&state.pool, &range)
        .await
...
{"timestamp":"2026-04-20T10:00:02Z","level":"WARN","target":"st0x_hedge","message":"Slow response"}"#;
    ...
    async fn performance_reliability_returns_empty_report_on_fresh_database() {
        let ctx = create_test_ctx_with_order_owner(Address::ZERO);
        let state = empty_app_state(ctx).await;
        crate::conductor::setup_apalis_tables(&state.pool)
            .await
            .unwrap();
        let app = build_app(state);
    ...
    async fn performance_reliability_reports_seeded_failures_and_jobs() {
        ...
        sqlx::query(
            "INSERT INTO events \
             (aggregate_type, aggregate_id, sequence, event_type, event_version, \
              payload, metadata) \
             VALUES ('OffchainOrder', '11111111-2222-3333-4444-555555555555', 1, \
              'OffchainOrderEvent::Failed', '1.0', $1, '{}')",
        )
        .bind(payload.to_string())
        .execute(&state.pool)
        .await
