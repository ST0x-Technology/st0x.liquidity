...
{"timestamp":"2026-04-20T10:00:02Z","level":"WARN","target":"st0x_hedge","message":"Slow response"}"#;
    ...
    async fn rebuild_waits_for_an_in_flight_projection_write() {
        ...
        writer.await.unwrap();
        match rebuild.await.unwrap() {
            Ok(_) => {}
            Err((status, Json(error))) => {
                panic!(
                    "the rebuild must run after the writer drains: {status}: {}",
                    error.error
                )
            }
        }
    ...
    async fn direct_http_projection_write_waits_for_rebuild_pause() {
        ...
        match tokio::time::timeout(std::time::Duration::from_millis(20), &mut write).await {
            Err(_) => {}
            Ok(_) => {
                panic!("a direct HTTP writer must park while rebuild maintenance is paused")
            }
        }
        ...
        match write.await.unwrap() {
            ...
            Err((status, Json(error))) => {
                panic!(
                    "the writer must resume after the rebuild: {status}: {}",
                    error.error
                )
            }
        }
