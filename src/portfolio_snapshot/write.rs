...
//!
//! **Scheduling semantics (see SPEC.md "Portfolio Capital and Return
//! Tracking"):** one snapshot per ET day, labeled by the ET day it is
...
//! [`exceeds_lateness_cap`]). That window is anchored to the target
...
/// [`exceeds_lateness_cap`] gives up and abandons the day entirely
...
impl PortfolioSnapshotJob {
    ...
    async fn perform_at(
        ...
        // Resolved once here and threaded through the lower-bound gate, the
        // lateness cap, and both capped retry backoffs below:
        // `target_et_day` never changes within a single `perform_at` call.
        ...
        if exceeds_lateness_cap(boundary, now) {
...
/// taken right after X's midnight, never an in-progress or stale mid-day read.
...
/// `boundary` local it threads through its lower-bound gate, lateness check,
/// and both [`capped_retry_backoff`] retries; [`first_capture`] resolves it
/// once the same way for its bootstrap-only checks.
...
/// capture-window boundary, [`MAX_FRESHNESS_DEFER`] past it. Accepting the
/// resolved `Option<DateTime<Utc>>` lets `perform_at` reuse one timezone
/// resolution and lets tests directly pin the DST-ambiguous `None` arm; no
/// real US Eastern calendar date makes `et_midnight` return `None`.
...
/// Test-side wrapper that resolves `target_et_day` before applying
/// [`exceeds_lateness_cap`], so date-based tests can pin the combined
/// resolve-and-check behavior.
#[cfg(test)]
...
/// [`exceeds_lateness_cap`]), so the LAST retry before abandonment lands
...
/// `exceeds_lateness_cap` never abandons in that case either; and `now`
...
/// fires 5 minutes from now abandons via `exceeds_lateness_cap` exactly
...
/// `today` even when `now` is already past today's boundary; always deferring
/// to `next_capture_delay(now)` would silently skip today on a restart after
/// the boundary. Targeting today does not by itself guarantee CAPTURING it,
/// though: `perform_at`'s
/// window gate ([`exceeds_lateness_cap`]) still abandons today's target
...
mod tests {
    ...
    /// `first_capture` must target today, not tomorrow, with the ordinary
    /// fixed backoff because there is far more than
    /// `HYDRATION_RETRY_BACKOFF` of window left.
    #[test]
    ...
    /// `now` close to the lateness cap: a bootstrap landing
    ...
    /// `now` before today's boundary: `first_capture` still targets today,
    /// just waits for the boundary rather than capturing early.
    #[test]
    ...
    /// The configured equity symbol is polled at MarketMaking but not at
    /// Hedging (USDC tracking disabled so only the equity gap is under test);
    /// a mere "rows non-empty" check would wrongly lock in an incomplete day.
    #[tokio::test]
    ...
    /// A position whose price is genuinely stale (over
    /// `MARK_STALENESS_THRESHOLD_DAYS` old) but was touched again today by an
    /// unrelated non-price event must still have its day excluded.
    ...
    async fn reschedules_at_next_et_midnight_plus_buffer_after_capture() {
        ...
        let expected_delay = (expected_boundary - now).to_std().unwrap();
        ...
        let rescheduled: PortfolioSnapshotJob = serde_json::from_slice(&job_bytes).unwrap();
        ...
        assert_eq!(rescheduled.target_et_day, tomorrow);
    }
    ...
    /// A job whose `target_et_day` is yesterday (hydration stuck past
    /// midnight, or a very late wake) must capture today's fresh balances
    /// under today's id -- never back-date a live read under yesterday's
    /// stale id. `build_fully_hydrated_ctx` marks every
    ...
    /// A job whose `target_et_day` has already passed (yesterday) and whose
    ...
    async fn perform_before_its_target_day_reschedules_without_capturing() {
        ...
        // the REAL wall clock at insertion time, not `now` -- only the
        // delay's LENGTH is derived from `now` (see
        // `reschedules_at_next_et_midnight_plus_buffer_after_capture`).
        let real_before = Utc::now();
        early_job
            .perform_at(&ctx, safe_capture_now())
            .await
        ...
        let expected_delay = et_midnight(future_day).unwrap() + CAPTURE_BUFFER - safe_capture_now();
        let expected = (real_before + expected_delay).timestamp();
        assert!(
            (run_at - expected).abs() <= 5,
    ...
    async fn bootstrap_purges_stale_pending_row_and_leaves_exactly_one() {
        ...
        // used below as the expected floor instead of a hardcoded duration.
        // `first_capture` caps that delay to the time remaining before the
        // lateness cap, so a fixed
        ...
        // Bootstrap must never enqueue a capture sooner than `first_capture`
        // itself would schedule it for -- ordinarily
        ...
        // cap. `run_at` is derived from a `Utc::now()` at or after `before`,
    ...
    /// The freshness-gap retry delay must be capped to the remaining time
    /// before the lateness cap, not the fixed
    ...
    /// `now` exactly at the boundary-anchored cap must still be within the
    /// window:
    ...
    /// Even when freshness genuinely passes -- simulating a same-day restart that
    ...
    /// A job whose `target_et_day` has already passed rebinds to today, and the
