...
async fn fail_offchain_order_aggregate<W: Write>(
    ...
    // the AlreadyCompleted handling below covers that remaining sliver for
    // complete fills.
    ...
    match send_result {
        ...
        Err(AggregateError::UserError(LifecycleError::Apply(
            ...
            match terminal {
                ...
                Some(Failed { .. } | Pending { .. } | Submitted { .. }) | None => {
                    writeln!(
                        stdout,
...
mod tests {
    ...
    async fn fail_pending_also_fails_offchain_order_aggregate() {
        ...
        // The OffchainOrder aggregate itself is now Failed -- no orphan.
        ...
        assert!(
            ...
            "OffchainOrder must be Failed, got {order:?}"
        );
    ...
    /// A concurrent PARTIAL fill in the same race window must also be caught:
    /// MarkFailed is legal from PartiallyFilled at the aggregate level, so
    /// without the pre-send re-load the executed shares would be erased
    /// silently.
    #[tokio::test]
