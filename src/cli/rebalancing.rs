...
mod tests {
    ...
    /// seeded as a raw event. Sequences are 1-based (the snapshot-aware loader
    /// reads `sequence > 0`).
    ...
    async fn fail_transfer_mint_force_fails_from_requested() {
        ...
        let pool = setup_test_db().await;
        let id = IssuerRequestId::new("cli-mint-requested");
        insert_mint_event(
            &pool,
            &id,
            1,
        ...
        assert_eq!(
            ...
            "Mint cli-mint-requested marked as failed\n",
