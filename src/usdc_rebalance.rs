...
mod tests {
    ...
    #[test]
    fn reservation_recovery_uses_nominal_amount_before_burn() {
        let nominal = Usdc::new(float!(321));
        let burn_amount = Usdc::new(float!(320));
        let state = UsdcRebalance::BridgingSubmitting {
            direction: RebalanceDirection::AlpacaToBase,
            amount: nominal,
            from_block: 100,
        ...
        let recovery = state
            .alpaca_to_base_reservation_recovery()
            .expect("pre-burn bridging must recover its source reservation");
        assert_eq!(
            recovery.amount, nominal,
            "settlement releases the nominal reservation; the pre-burn recovery must not use \
             the withdrawal-fee-adjusted burn amount"
        );
    }

    #[test]
    fn reservation_recovery_rejects_onchain_withdrawal_reference() {
        ...
        assert_eq!(
            state.alpaca_to_base_reservation_recovery(),
            None,
            "only Alpaca transfer IDs have a startup redrive path"
        );
    }
