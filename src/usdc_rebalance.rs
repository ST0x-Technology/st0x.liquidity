...
pub struct UnknownRebalanceDirection(String);
...
impl ConversionAmounts {
    pub const fn new(source_amount: Usdc, received_amount: Usdc) -> Self {
        Self {
            source_amount,
            received_amount,
        }
...
pub enum UsdcRebalanceCommand {
    ...
    /// same-amount send. Valid only from a BaseToAlpaca `Bridged`.
...
pub enum UsdcRebalance {
    ...
    Bridged {
        ...
        initiated_at: DateTime<Utc>,
        minted_at: DateTime<Utc>,
        /// BaseToAlpaca deposit send tx recorded at broadcast by
        /// `RecordPendingDeposit`. `None` until the send is broadcast, and for
...
impl EventSourced for UsdcRebalance {
    ...
    // v9: AlpacaToBase in-flight states persist the preflight Ethereum USDC
    ...
    async fn initialize(
        ...
        match command {
            ...
            InitiateDeposit { .. } | RecordPendingDeposit { .. } => {
...
mod tests {
    ...
    // The resume path segment is a wire contract: the CLI renders `Display`
    // and the server route parses `FromStr`. Pin both literals and both
    // conversion directions so neither side can drift alone.
    #[test]
    fn rebalance_direction_wire_literals_round_trip() {
        assert_eq!(
            RebalanceDirection::AlpacaToBase.to_string(),
            "alpaca_to_base"
        );
        assert_eq!(
            RebalanceDirection::BaseToAlpaca.to_string(),
            "base_to_alpaca"
        );
        assert_eq!(
            "alpaca_to_base".parse::<RebalanceDirection>().unwrap(),
            RebalanceDirection::AlpacaToBase
        );
        assert_eq!(
            "base_to_alpaca".parse::<RebalanceDirection>().unwrap(),
            RebalanceDirection::BaseToAlpaca
        );

        let error = "sideways".parse::<RebalanceDirection>().unwrap_err();
        assert!(matches!(error, UnknownRebalanceDirection(_)));
    }
    ...
    /// AlpacaToBase's deposit is the bot's own on-chain tx: reconcile
    /// territory (`transfer reconcile --kind usdc`; resume returns
    /// `PreviouslyFailedAggregate` for `DepositFailed`), not provider
    /// recheck. Recovery must refuse rather than confirm a deposit no
    /// provider ever settled.
    #[tokio::test]
    ...
    fn ethereum_wallet_credit_covers_only_credited_unsent_usdc() {
        ...
        // A recorded deposit send may already have left the wallet.
        assert_eq!(
            Bridged {
                ...
                initiated_at: now,
                minted_at: now,
                pending_deposit_tx: Some(BURN_TX),
            }
    ...
    async fn record_pending_deposit_keeps_the_send_tx_on_bridged() {
        ...
        let [UsdcRebalanceEvent::PendingDepositRecorded { send_tx, .. }] = events.as_slice() else {
            panic!("Expected one PendingDepositRecorded event, got {events:?}");
        };
        assert_eq!(*send_tx, DEPOSIT_SEND_TX);

        let state = replay::<UsdcRebalance>([staged, events].concat()).unwrap();
        let Some(UsdcRebalance::Bridged {
            pending_deposit_tx,
            amount_received,
            ..
        }) = state
        ...
        assert_eq!(pending_deposit_tx, Some(DEPOSIT_SEND_TX));
        assert_eq!(amount_received, Usdc::new(float!(99.99)));
    }

    #[tokio::test]
    async fn record_pending_deposit_rejects_an_alpaca_to_base_transfer() {
        let error = TestHarness::<UsdcRebalance>::with(())
    ...
    async fn fail_deposit_from_bridged_keeps_the_recorded_send() {
        let mut staged = bridged_events(RebalanceDirection::BaseToAlpaca);
        staged.push(UsdcRebalanceEvent::PendingDepositRecorded {
            send_tx: DEPOSIT_SEND_TX,
            recorded_at: Utc::now(),
        });
    ...
    async fn fail_deposit_from_bridged_without_a_recorded_send_has_no_deposit_ref() {
        let events = TestHarness::<UsdcRebalance>::with(())
            .given(bridged_events(RebalanceDirection::BaseToAlpaca))
            .when(UsdcRebalanceCommand::FailDeposit {
                reason: "unrecorded send found".to_string(),
            })
            .await
            .events();
    ...
    #[test]
    fn bridged_snapshot_without_pending_deposit_tx_deserializes_to_none() {
        let now = Utc::now();
        let mut snapshot = to_value(UsdcRebalance::Bridged {
            ...
            initiated_at: now,
            minted_at: now,
            pending_deposit_tx: Some(DEPOSIT_SEND_TX),
        })
        .unwrap();
        snapshot
            .get_mut("Bridged")
        ...
        assert_eq!(pending_deposit_tx, None);
    }
}
