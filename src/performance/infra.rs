...
async fn current_lag(
    ...
    Ok(match latest {
        Some((raw_sampled_at, lag_blocks)) => (Some(lag_blocks), parse_timestamp(&raw_sampled_at)),
        None => (None, None),
    })
}
...
async fn poll_health(
    ...
    let rows: Vec<PollCycleRow> = sqlx::query_as(
        "SELECT duration_ms, skipped_ticks, outcome FROM poll_cycle_samples \
    ...
    let skipped_ticks = rows.iter().map(|row| count(row.skipped_ticks)).sum();
    let durations: Vec<i64> = rows.iter().map(|row| row.duration_ms).collect();

    Ok(PollHealth {
        cycles,
        errors,
        skipped_ticks,
        duration: latency_stats(durations),
    })
}

/// One `poll_cycle_samples` row, named so the SELECT column order cannot
/// silently bind to the wrong field.
#[derive(sqlx::FromRow)]
struct PollCycleRow {
    duration_ms: i64,
    skipped_ticks: i64,
...
mod tests {
    ...
    async fn reports_latest_lag_and_bucketed_maxima() {
        ...
        insert_lag(&pool, 10, 110, Some(100)).await; // confirmed 107, lag 7
        insert_lag(&pool, 20, 125, Some(100)).await; // confirmed 122, lag 22
        insert_lag(&pool, 4_000, 210, Some(205)).await; // confirmed 207, lag 2
    ...
    async fn current_lag_ignores_range_but_buckets_respect_it() {
        ...
        // the assertion can tell which sample won: confirmed 497, lag 2.
        insert_lag(&pool, 100_000, 500, Some(495)).await;
        // In range: confirmed 107, lag 7.
