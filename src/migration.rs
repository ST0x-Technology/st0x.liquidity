...
pub async fn migrate_schwab_auth(
    ...
    match execution {
        ExecutionMode::Commit => {
            persist_schwab_auth_event(pool, aggregate_id, event).await?;
        }
        ExecutionMode::DryRun => {}
    }
...
async fn persist_schwab_auth_event(
    pool: &SqlitePool,
    aggregate_id: &str,
...
mod tests {
    ...
    async fn test_migrate_schwab_auth_with_tokens() {
        ...
        let event_type: String =
            sqlx::query_scalar("SELECT event_type FROM events WHERE aggregate_type = 'SchwabAuth'")
                .fetch_one(&pool)
                .await
                .unwrap();
