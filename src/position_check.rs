...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum FailedAnchorRecoveryAction {
    Preserve,
    Release,
    Enqueue,
}

pub(crate) fn failed_anchor_recovery_action(
...
impl HedgeScanSkipReason {
    pub(crate) const fn metric_label(self) -> &'static str {
        match self {
            Self::AnchoredOrder => "anchored_order",
    ...
    E: Executor + Clone + Send + Sync + 'static,
{
    ...
    async fn scan_and_enqueue(
        ...
        // broker re-drive serializes against live placements.
        {
            ...
            let _file_submission_guard =
                acquire_counter_trade_submission_file_lock(&self.pool).await?;
        ...
        // Every unanchored symbol starts the scan at zero deficit; the
        // preflight below overwrites the ones it tries to hedge. Anchored
        // symbols retain their actual deficit while recovery owns the order.
        // Otherwise a symbol that stops being hedge-ready keeps its last
        // deficit until restart.
        for (symbol, position) in &all_positions {
            let requested = if position.last_failed_offchain_order_id.is_some() {
                let Ok(requested) = position.absolute_net_shares().inspect_err(
                    |error| error!(%symbol, %error, "Failed to export anchored hedge deficit"),
                ) else {
                    continue;
                };
                requested
    ...
    /// the scan-time quantity. Placement repeats this check while holding the
    /// submission lock and authoritatively sizes against then-current funds.
    async fn preflight_allows_enqueue(
        &self,
        ready: &ExecutionCtx,
        close_flatten_window_cache: &mut CloseFlattenWindowCache,
    ) -> bool {
...
mod tests {
    ...
    async fn close_flatten_buy_preflight_partially_hedges_at_the_ask() {
        ...
        assert_eq!(
            ...
            "scan-time affordability must not cap the quantity rechecked under the placement lock"
        );
        let rendered = metrics_handle.render();
        assert!(
            !rendered.contains("close_flatten_blocked_total{"),
    ...
    async fn ordinary_extended_hours_buy_partially_hedges_at_the_crossed_mark() {
        ...
        assert_eq!(
            jobs[0].shares,
            Positive::new(FractionalShares::new(float!(2.0))).unwrap(),
            "the job must retain the full requested quantity for authoritative placement-time sizing"
        );
        let rendered = metrics_handle.render();
        assert!(
            !rendered.contains("close_flatten_blocked_total{"),
    ...
    fn hedge_scan_skip_reason_labels_are_stable() {
        assert_eq!(
            [
                HedgeScanSkipReason::AnchoredOrder,
