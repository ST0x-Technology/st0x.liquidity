...
use crate::trading::offchain::hedge::{
    acquire_counter_trade_submission_file_lock, apply_slippage, push_anchor_recovery_job_if_absent,
    resolve_extended_hours_reference_price,
};
use crate::trading::onchain::exclusion::{ExclusionCause, record_trading_enablement};
...
async fn setup_onchain_trade_store(
    ...
    // The PnL ledger books excluded fills and their covers from this
    // aggregate's events.
...
async fn finish_startup_recovery(deps: StartupRecoveryDeps<'_>) -> anyhow::Result<()> {
    requeue_startup_orphans(deps.schedulers, deps.backfill_queues).await?;
...
/// Builds the instrumented executor, the per-chain providers, and the
/// telemetry writer, then runs the startup smoke checks against exactly that
/// set -- so `run` only ever receives infrastructure that has already proven
/// it can reach its chains and broker.
...
impl Conductor {
        ...
        crate::offchain::order::JobError: From<E::Error>,
    {
        ...
        let onchain_trade = setup_onchain_trade_store(
            &pool,
            dashboard_delivery.broadcaster.clone(),
            pnl_ledger_reactor.clone(),
        )
        .await?;
        ...
        run_startup_maintenance(&ctx, &pool, &provider, &watch_providers, &onchain_trade).await?;
...
async fn setup_bot_gas_receipt_cost(
    ...
    let record_ctx = build_record_bot_gas_receipt_cost_ctx(ctx, store, queue.clone())?;

    Ok((queue, record_ctx))
}

/// Builds the [`RecordBotGasReceiptCostCtx`] (ADR 0020). `[bot_gas_valuation]`
...
async fn run_startup_maintenance<P: Provider>(
    ctx: &Ctx,
    pool: &SqlitePool,
    primary_provider: &P,
    ...
    // Before any fill is accounted: a fill that landed while its asset was
    // disabled stays excluded from hedging after the asset is enabled. The
    // boundaries are each chain's head block as read now.
    let heads = read_hedged_chain_heads(ctx, primary_provider, watch_providers).await?;
    record_trading_enablement(pool, &ctx.chains, &heads, Utc::now())
        .await
        .context("failed to record the trading flags observed at startup")?;

    let adopted = adopt_legacy_exclusions(pool, onchain_trade).await?;
    if adopted > 0 {
        info!(
            adopted,
            "Adopted fills excluded before exclusions were recorded on their trades"
        );
    }
...
/// Each hedged chain's current head block, the boundary between fills that
/// landed before this restart and fills that land under the flags it observes.
async fn read_hedged_chain_heads<P: Provider>(
    ctx: &Ctx,
    primary_provider: &P,
    watch_providers: &BTreeMap<Chain, P>,
) -> anyhow::Result<BTreeMap<Chain, u64>> {
    let mut heads = BTreeMap::new();
    for hedged in ctx.chains.hedged() {
        let provider = if hedged.chain == ctx.chains.primary().chain {
            primary_provider
        } else {
            watch_providers
                .get(&hedged.chain)
                .with_context(|| format!("no provider for hedged chain {}", hedged.chain))?
        };
        let head = provider
            .get_block_number()
            .await
            .with_context(|| format!("failed to read the head block of {}", hedged.chain))?;
        heads.insert(hedged.chain, head);
    }
...
async fn recover_interrupted_tokenization_aggregates(
    ...
    for generation in transfer_mints
        .iter()
        .map(|row| row.task.generation)
        .chain(transfer_redemptions.iter().map(|row| row.task.generation))
    {
        GUARD_GENERATION.reserve(generation);
    }
    ...
    // The bulk event queries already identify recoverable aggregates. Finished
    // historical jobs need no individual aggregate replay to restore ownership.
    let interrupted_mints: HashSet<_> = interrupted_mints.iter().collect();
    let interrupted_redemptions: HashSet<_> = interrupted_redemptions.iter().collect();
    transfer_mints.retain(|row| {
        !row.is_terminal() || interrupted_mints.contains(&row.task.issuer_request_id)
    });
    transfer_redemptions.retain(|row| {
...
impl<Task> DurableTransferJob<Task> {
    fn is_terminal(&self) -> bool {
        self.status == Status::Done
            || self.status == Status::Killed
            || (self.status == Status::Failed && self.attempts >= self.max_attempts)
    }
}

async fn load_transfer_jobs<Task>(
    pool: &SqlitePool,
) -> anyhow::Result<Vec<DurableTransferJob<Task>>>
where
    Task: serde::de::DeserializeOwned + 'static,
{
    let rows: Vec<(Vec<u8>, String, i64, i64)> =
        sqlx::query_as("SELECT job, status, attempts, max_attempts FROM Jobs WHERE job_type = ?")
            .bind(std::any::type_name::<Task>())
            .fetch_all(pool)
            .await?;

    rows.into_iter()
        .map(|(payload, status, attempts, max_attempts)| {
            let task = serde_json::from_slice(&payload).with_context(|| {
                format!(
                    "failed to deserialize durable {} transfer job",
                    std::any::type_name::<Task>()
                )
            })?;
            Ok(DurableTransferJob {
                task,
                status: status.parse().with_context(|| {
                    format!(
                        "invalid durable {} transfer job status",
                        std::any::type_name::<Task>()
                    )
                })?,
                attempts,
        ...
        if !row.is_terminal() {
            insert_transfer_owner(
                ...
                TransferGuardOwner {
                    ...
                    job_type: std::any::type_name::<TransferEquityToMarketMaking>(),
                    target: ResumeTokenizationTarget::Mint(row.task.issuer_request_id.clone()),
                },
            )?;
        }
    }
    ...
    for row in redemptions {
        ...
        if !row.is_terminal() {
            insert_transfer_owner(
                ...
                TransferGuardOwner {
                    generation: row.task.generation,
                    job_type: std::any::type_name::<TransferEquityToHedging>(),
                    target: ResumeTokenizationTarget::Redemption(row.task.aggregate_id.clone()),
                },
            )?;
        }
    }
    ...
    for (symbol, owner) in guard_owners {
        ...
        if generation.is_legacy() {
            warn!(
                target: "rebalance",
                %symbol,
                "Restoring a live legacy equity transfer with its reserved zero generation"
            );
        }

        guard.insert(symbol, GuardState::ActiveTransfer { generation });
    }
    drop(guard);
...
struct TransferGuardOwner {
    ...
    job_type: &'static str,
    target: ResumeTokenizationTarget,
}

fn insert_transfer_owner(
    owners: &mut HashMap<Symbol, TransferGuardOwner>,
    symbol: &Symbol,
    owner: TransferGuardOwner,
) -> anyhow::Result<()> {
...
pub(crate) async fn recover_orphaned_pending_offchain_orders(
    ...
    let orphaned: Vec<_> = positions
        ...
        .filter_map(|(symbol, pos)| {
            pos.pending_offchain_order_id.map(|order_id| {
                let missing_order_anchor = if pos.last_failed_offchain_order_id.is_some() {
                    AnchorDisposition::Preserve
                } else {
                    AnchorDisposition::Release
                };
                (symbol, order_id, missing_order_anchor)
            })
        })
        .collect();
    ...
    for (symbol, order_id, missing_order_anchor) in orphaned {
        recover_single_orphaned_order(
            position,
            offchain_order,
            order_placer,
            &symbol,
...
async fn recover_single_orphaned_order(
    ...
    missing_order_anchor: AnchorDisposition,
    configured_executor: SupportedExecutor,
) -> anyhow::Result<()> {
        ...
        Some(OffchainOrder::Pending {
            ...
            if executor != configured_executor {
                warn!(
                    ...
                    %configured_executor,
                    "Leaving pending offchain order unchanged because its executor is not configured"
                );
                return Ok(());
            }
...
///
/// A trade already acknowledged by an exclusion was kept out of hedging by a
/// concurrent excluded run while this run applied it to `Position`: that is a
/// conflict, never this run's own marker, so it fails loudly.
pub async fn execute_mark_acknowledged(
        ...
        .await
    {
        ...
        Err(AggregateError::UserError(LifecycleError::Apply(
            ...
            Some(state) if state.is_excluded() => Err(TradeAccountingError::ExclusionConflict {
...
/// Marks the trade excluded from hedging, which also acknowledges it, so a
/// redrive does nothing. A re-driven marker on an excluded trade is
/// idempotent. A trade acknowledged without an exclusion was accounted into
/// `Position` by a concurrent hedged run: that is a conflict, never a
/// successful exclusion, so it fails loudly instead of paging a cover for a
/// fill the bot also hedges. Infrastructure errors propagate so the apalis
/// retry re-drives the idempotent exclusion steps.
pub async fn execute_mark_excluded(
    onchain_trade: &Store<OnChainTrade>,
    trade_id: &OnChainTradeId,
) -> Result<(), TradeAccountingError> {
    match onchain_trade
        .send(trade_id, OnChainTradeCommand::Exclude)
        .await
    {
        Ok(()) => Ok(()),
        Err(AggregateError::UserError(LifecycleError::Apply(
            ...
            Some(state) if state.is_excluded() => Ok(()),
            _ => Err(TradeAccountingError::ExclusionConflict {
                trade_id: trade_id.clone(),
            }),
        },
        Err(error) => Err(error.into()),
...
/// Adopts fills excluded before exclusions were recorded on `OnChainTrade`:
/// a `trading_disabled` row in `skipped_fills` whose trade is acknowledged
/// but carries no exclusion. Each gets its `ExcludedFromHedging` event, so it
/// is listed as uncovered, counted in pages, can take a manual cover and is
...
pub(crate) async fn adopt_legacy_exclusions(
    ...
    let mut adopted = 0;
    for (chain, tx_hash, log_index) in candidates {
        let trade_id: OnChainTradeId = format!("{chain}:{tx_hash}:{log_index}")
            .parse()
        ...
        if position_fill_already_recorded(pool, &state.symbol, &trade_id).await? {
            warn!(
                %trade_id,
                "Fill recorded as trading disabled is already in the position; not adopting it \
                 as excluded"
            );
            continue;
        }
        ...
        match onchain_trade
            .send(&trade_id, OnChainTradeCommand::AdoptLegacyExclusion)
            .await
        {
            Ok(()) => adopted += 1,
            Err(AggregateError::UserError(LifecycleError::Apply(
                OnChainTradeError::AlreadyExcluded,
            ))) => {}
            Err(error) => {
                return Err(anyhow::Error::new(error)
                    .context(format!("failed to adopt excluded fill {trade_id}")));
            }
        }
    }
...
pub enum FillAccountingOutcome {
    AlreadyAcknowledged,
    /// Excluded from hedging and recorded in `skipped_fills`, because trading
    /// was disabled for it on its chain. Never hedged: an operator covers its
    /// delta by hand, even after trading is enabled again. `detail` is the
...
/// The recorded exclusion detail of an acknowledged fill, when it was excluded
/// from hedging: a `trading_disabled` record on a fill that never reached
/// `Position` (an exclusion, or a legacy one not adopted yet).
///
...
pub async fn account_for_onchain_fill(
    ...
    let (trade_id, block_timestamp) =
        match witness_onchain_fill(onchain_trade, trade, block_number).await? {
            WitnessedFill::Pending {
                trade_id,
                block_timestamp,
...
///
/// `cause` is why the fill is excluded, from
/// [`crate::trading::onchain::exclusion::exclusion_cause`]. `event_type`
/// names what surfaced the fill, for the skipped fill record.
pub async fn account_for_fill_excluded_from_hedging(
    ...
    let cover = cover_direction(trade.direction);
    let detail = format!(
        "onchain fill {direction} {amount} {symbol} at {price} USDC on {chain}, \
         cover by {cover} {amount} {symbol} at the broker: {cause} for {symbol} \
         on {chain}, so the fill is not counter traded",
        direction = trade.direction,
        amount = trade.amount,
    ...
    match reason {
        ...
        CounterTradeSkipReason::BelowBrokerPrecision {
            ...
            debug!(
                ...
                source,
                quantity_decimals,
                "Deferring counter trade until its quantity reaches the broker precision"
            );
        }
...
/// Returns whether a failed anchor blocks fresh placement. A matching Alpaca
/// anchor is queued for recovery; a mismatched executor anchor is preserved and
/// also blocks, while a matching DryRun anchor is safe to release.
async fn handle_failed_anchor_recovery(
    symbol: &Symbol,
    configured_executor: SupportedExecutor,
    cqrs: &TradeProcessingCqrs,
    ...
    match failed_anchor_recovery_action(configured_executor, anchored_order.executor()) {
...
async fn recover_claimed_offchain_order_for_symbol(
    ...
    let missing_order_anchor = if position.last_failed_offchain_order_id.is_some() {
        AnchorDisposition::Preserve
    } else {
        AnchorDisposition::Release
    };
...
mod tests {
    ...
    async fn onchain_trade_store_enqueues_terminal_dashboard_delivery() {
        ...
        let store = setup_onchain_trade_store(&pool, broadcaster, test_pnl_ledger_reactor(&pool))
            .await
            .unwrap();
    ...
    async fn seed_mint_to_tokens_received(
        store: &Store<TokenizedEquityMint>,
        id: &IssuerRequestId,
        symbol: &Symbol,
        wallet: Address,
    ) {
    ...
    #[tokio::test]
    async fn live_transfer_owner_suppresses_generic_resume_for_its_aggregate() {
        let InterruptedAggregateFixture {
            pool,
            apalis_pool,
            services,
    ...
    #[tokio::test]
    async fn durable_transfer_rejects_unknown_queue_status() {
        let (pool, apalis_pool) = setup_test_pools().await;
        let mut queue =
            crate::rebalancing::equity::TransferEquityToMarketMakingJobQueue::new(&apalis_pool);
        queue
            ...
            .await
            .unwrap();
        sqlx_apalis::query("UPDATE Jobs SET status = 'Unknown' WHERE job_type = ?")
            .bind(std::any::type_name::<TransferEquityToMarketMaking>())
            .execute(&apalis_pool)
            .await
            .unwrap();
        let error = load_transfer_jobs::<TransferEquityToMarketMaking>(&pool)
            .await
            .err()
            .expect("unknown queue status must fail startup");
        assert!(format!("{error:#}").contains("Unknown state: Unknown"));
    }

    #[tokio::test]
    async fn mixed_direction_legacy_transfer_rows_for_one_symbol_fail_startup() {
        let InterruptedAggregateFixture {
            pool,
            apalis_pool,
            services,
        ...
        assert!(
            error
                .to_string()
                .contains("multiple live equity transfer rows own symbol AAPL"),
            "startup must reject competing live transfer owners, got: {error:#}"
        );
        let message = error.to_string();
        for owner in [
            std::any::type_name::<TransferEquityToMarketMaking>().to_owned(),
            std::any::type_name::<TransferEquityToHedging>().to_owned(),
            issuer_request_id("transfer-owned-mint").to_string(),
            redemption_aggregate_id("transfer-owned-redemption").to_string(),
        ] {
            assert!(
                message.contains(&owner),
                "missing owner {owner} in {message}"
            );
        }
    }
    ...
    #[tokio::test]
    async fn live_pre_wrap_transfer_rows_replace_recovery_holds_on_restart() {
        let InterruptedAggregateFixture {
            pool,
            apalis_pool: _,
            services,
            ...
            "pre-wrap-owner-existing-redemption",
        )
        .await;
        let symbol = Symbol::new("AAPL").unwrap();
        ...
        mint_store
            .send(
                ...
                TokenizedEquityMintCommand::FailAcceptance {
                    reason: "terminal before queue acknowledgement".to_string(),
                },
            )
            .await
            .unwrap();
    ...
    async fn terminal_transfer_with_recoverable_aggregate_retains_recovered_guard() {
        ...
        let status: String =
            sqlx_apalis::query_scalar("SELECT status FROM Jobs WHERE job_type = ?")
                ...
                .await
                .unwrap();
        assert_eq!(status, Status::Failed.to_string());
    }
    ...
    async fn terminal_redemption_retains_recovered_guard_without_generic_resume() {
        let mut fixture = seed_interrupted_aggregates_and_build_service(
            5,
            "unowned-mint",
            "dead-lettered-legacy-redemption",
        )
        .await;
        let mut queue =
    ...
    async fn recover_interrupted_tokenization_aggregates_excludes_held_for_recovery_pre_wrap_mint()
    {
        ...
        seed_mint_to_tokens_received(
            &seeding_mint_store,
            &mint_id,
            &symbol,
            alloy::primitives::Address::from([3_u8; 20]),
        )
        .await;
        ...
        seed_mint_to_tokens_received(
            ...
            &mint_id2,
            &symbol,
            alloy::primitives::Address::from([4_u8; 20]),
        )
        .await;
    ...
    #[tokio::test]
    async fn inline_recovery_preserves_alpaca_anchor_when_dry_run_is_configured() {
        let (pool, apalis_pool) = setup_test_pools().await;
        let (frameworks, _) = create_cqrs_frameworks(&pool).await;
        let (cqrs, _) = trade_processing_cqrs_with_threshold(
            &frameworks,
            &pool,
            ExecutionThreshold::whole_share(),
            &apalis_pool,
        ...
        acknowledge_fill(&cqrs.position, "AAPL", "2", Direction::Sell, 1).await;
        cqrs.offchain_order
            .send(
                &anchor,
        ...
        assert!(
            handle_failed_anchor_recovery(&symbol, SupportedExecutor::DryRun, &cqrs)
                .await
                .unwrap()
        );
        let position = cqrs.position.load(&symbol).await.unwrap().unwrap();
    ...
    async fn excluded_path_finishes_fill_already_applied_to_position() {
        ...
        let outcome = account_for_fill_excluded_from_hedging(
            ...
            &trade,
            trade_event.block_number,
            ExclusionCause::TradingDisabled,
            "InventoryTrade",
        )
        .await
        .unwrap();
    ...
    /// A fill excluded before exclusions were recorded on the trade (only an
    /// `Acknowledged` marker and a `trading_disabled` row) is adopted once at
    /// startup, so it can take a cover and reach the PnL ledger. A fill the
    /// position holds is left alone.
    #[tokio::test]
    async fn startup_adopts_legacy_exclusions_once_and_skips_hedged_fills() {
        let (pool, apalis_pool) = setup_test_pools().await;
        let (frameworks, _offchain_order_projection) = create_cqrs_frameworks(&pool).await;
        let (cqrs, _assets) = trade_processing_cqrs_with_threshold(
            &frameworks,
        ...
        let trade_event = make_trade_event(60);
        let legacy = test_trade_with_amount(float!(1.5), 60);
        let hedged = test_trade_with_amount(float!(2), 61);

        for trade in [&legacy, &hedged] {
            let trade_id = OnChainTradeId {
                chain: trade.chain,
                tx_hash: trade.tx_hash,
                log_index: trade.log_index,
        ...
        let load = |trade: &crate::onchain::OnchainTrade| OnChainTradeId {
            chain: trade.chain,
            tx_hash: trade.tx_hash,
            log_index: trade.log_index,
        };
        let legacy_state = cqrs
            .onchain_trade
            .load(&load(&legacy))
            .await
            .unwrap()
            .unwrap();
        assert!(legacy_state.is_excluded());
        let hedged_state = cqrs
            .onchain_trade
            .load(&load(&hedged))
            .await
            .unwrap()
            .unwrap();
        assert!(
            !hedged_state.is_excluded(),
            "a fill in the position was hedged and must not take a manual cover"
        );
    }

    /// The exclusion path refuses to finish a fill a concurrent hedged run
    /// already acknowledged into the position, instead of treating the marker
    /// as its own and paging a manual cover for a hedged fill.
    #[tokio::test]
    async fn exclusion_of_a_fill_hedged_concurrently_is_a_conflict() {
        let (pool, apalis_pool) = setup_test_pools().await;
        let (frameworks, _offchain_order_projection) = create_cqrs_frameworks(&pool).await;
        let (cqrs, _assets) = trade_processing_cqrs_with_threshold(
            &frameworks,
        ...
        let error = execute_mark_excluded(&cqrs.onchain_trade, &trade_id)
            .await
            .unwrap_err();
        assert!(
            matches!(error, TradeAccountingError::ExclusionConflict { .. }),
            "{error:?}"
        );
    }

    /// A fill left both in `Position` and excluded by a race fails only the run
    /// that meets it: later deliveries through either path report it as hedged
    /// instead of failing again, and it is never listed as owed a cover.
    #[tokio::test]
    async fn later_deliveries_of_a_fill_classified_both_ways_report_it_hedged() {
        let (pool, apalis_pool) = setup_test_pools().await;
        let (frameworks, _offchain_order_projection) = create_cqrs_frameworks(&pool).await;
        let (cqrs, _assets) = trade_processing_cqrs_with_threshold(
            &frameworks,
        ...
        .await
        .unwrap();
        execute_mark_excluded(&cqrs.onchain_trade, &trade_id)
            .await
            .unwrap();

        let hedged = account_for_onchain_fill(
            &pool,
            &cqrs.onchain_trade,
            &cqrs.position,
            &trade,
        ...
        .await
        .unwrap();
        assert!(matches!(hedged, FillAccountingOutcome::AlreadyAcknowledged));

        let excluded = account_for_fill_excluded_from_hedging(
            &pool,
            &cqrs.onchain_trade,
            &cqrs.position,
            &trade,
        ...
        .await
        .unwrap();
        assert!(matches!(excluded, ExcludedFillOutcome::AlreadyAcknowledged));

        let uncovered = crate::trading::onchain::skipped_fill::uncovered_excluded_fills(
            &pool,
            trade.chain,
            trade.symbol.base().as_str(),
        )
        .await
        .unwrap();
        assert!(
    ...
    /// so the hedged run that applied it to `Position` must not take the
    /// marker as its own.
    #[tokio::test]
    async fn hedged_marker_on_a_fill_excluded_concurrently_is_a_conflict() {
        let (pool, apalis_pool) = setup_test_pools().await;
        let (frameworks, _offchain_order_projection) = create_cqrs_frameworks(&pool).await;
        let (cqrs, _assets) = trade_processing_cqrs_with_threshold(
            &frameworks,
        ...
        let error = execute_mark_acknowledged(&cqrs.onchain_trade, &trade_id)
            .await
            .unwrap_err();
        assert!(
            matches!(error, TradeAccountingError::ExclusionConflict { .. }),
    ...
    async fn hedged_path_finishes_fill_already_recorded_as_trading_disabled() {
        ...
        assert!(
            marked.is_excluded(),
            "the hedged path must record the exclusion, which the ledger, the uncovered set \
             and the cover route read"
        );
    }
    ...
    async fn extended_hours_trade_preserves_requested_quantity_in_immediate_hedge_job() {
        ...
        assert_eq!(
            job.shares,
            Positive::new(FractionalShares::new(float!(5.0))).unwrap(),
            "enqueue-time inventory is only a filter; placement-time preflight must size the job"
        );
    }
    ...
    #[tokio::test]
    async fn recover_orphaned_pending_preserves_anchor_for_missing_recovery_intent() {
        let pool = setup_test_db().await;
        let (position, position_projection) = StoreBuilder::<Position>::new(pool.clone())
            .build(())
            .await
        ...
        let recovered = position_projection.load(&symbol).await.unwrap().unwrap();
        assert_eq!(recovered.pending_offchain_order_id, None);
        assert_eq!(recovered.last_failed_offchain_order_id, Some(anchor));
        assert_eq!(
            client_order_id_for_placement(
                OffchainOrderId::new(),
                recovered.last_failed_offchain_order_id
            ),
            ClientOrderId::from_uuid(anchor.as_uuid())
        );
    }
    ...
    #[tokio::test]
    async fn recover_orphaned_pending_redrives_matching_executor_and_preserves_mismatch() {
        let pool = setup_test_db().await;
        ...
        let mismatched_symbol = Symbol::new("MSTR").unwrap();
        let mismatched_order_id = OffchainOrderId::new();
        position
            .send(
                &mismatched_symbol,
                PositionCommand::AcknowledgeOnChainFill {
                    symbol: mismatched_symbol.clone(),
                    threshold,
                    trade_id: TradeId {
                        chain: Chain::Base,
                        tx_hash: fixed_bytes!(
        ...
        position
            .send(
                &mismatched_symbol,
                PositionCommand::PlaceOffChainOrder {
                    offchain_order_id: mismatched_order_id,
                    shares,
                    direction: Direction::Sell,
        ...
        offchain_order
            .send(
                ...
                OffchainOrderCommand::Place {
                    ...
                    direction: Direction::Sell,
                    executor: SupportedExecutor::DryRun,
                    client_order_id: ClientOrderId::from_uuid(mismatched_order_id.as_uuid()),
                    kind: CounterTradeOrderKind::Market,
                },
            )
            .await
        ...
        assert!(matches!(
            offchain_order.load(&mismatched_order_id).await.unwrap(),
            Some(OffchainOrder::Pending { .. })
        ));
        assert_eq!(
            position_projection
                .load(&mismatched_symbol)
                .await
                .unwrap()
                .unwrap()
                .pending_offchain_order_id,
            Some(mismatched_order_id),
            "a pending order for an unconfigured executor must remain preserved"
        );
    }
    ...
    #[tokio::test]
    async fn recover_claimed_offchain_order_preserves_pending_executor_mismatch() {
        let (pool, apalis_pool) = setup_test_pools().await;
        let (frameworks, _projection) = create_cqrs_frameworks(&pool).await;
        let (cqrs, _assets) = trade_processing_cqrs_with_threshold(
            &frameworks,
        ...
        assert_eq!(result, Some(offchain_order_id));
        assert!(matches!(
            frameworks
                .offchain_order
                .load(&offchain_order_id)
        ...
        assert_eq!(
            frameworks
                ...
                .pending_offchain_order_id,
            Some(offchain_order_id),
            "an order for an unconfigured executor must remain claimed without broker re-drive"
        );
    }
