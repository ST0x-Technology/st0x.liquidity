...
pub enum FillMarkOutcome {
    ...
    /// applied is left for the reorg reversal flow to reverse.
    Reorged,
}

/// Marks the trade acknowledged on the `OnChainTrade` aggregate after the
/// position write succeeds, completing the exactly-once pair (ADR 0005).
...
pub async fn account_for_onchain_fill(
    ...
    match onchain_trade.load(&trade_id).await {
        Ok(Some(state)) => {
            ...
            // A reorg invalidated this fill before it was acknowledged. Skip
            // it as terminal rather than applying position impact and then
            // failing the acknowledgement on every retry.
            if state.is_reorged() {
                warn!(
                    ?trade_id,
                    symbol = %trade.symbol,
