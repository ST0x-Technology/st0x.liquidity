...
mod tests {
    ...
    async fn records_convert_usdc_usd_as_broker_operation() {
        ...
        assert_eq!(rows[0].1, "convert_usdc_usd");
    ...
    async fn records_find_conversion_order_as_broker_operation() {
        ...
        assert_eq!(rows.len(), 1);
        assert_eq!(rows[0].0, "broker");
        assert_eq!(rows[0].1, "find_conversion_order");
        assert_eq!(rows[0].2, "ok");
        assert_eq!(rows[0].3, None);
    }
    ...
    async fn records_poll_conversion_to_terminal_as_broker_operation() {
        ...
        assert_eq!(rows.len(), 1);
        assert_eq!(rows[0].0, "broker");
        assert_eq!(rows[0].1, "poll_conversion_to_terminal");
        assert_eq!(rows[0].2, "ok");
        assert_eq!(rows[0].3, None);
    }
    ...
    async fn records_convert_usdc_usd_error_as_broker_operation() {
        ...
        assert_eq!(
            rows[0].3.as_deref(),
            Some(format!("Crypto order {TEST_ORDER_ID} failed: Canceled").as_str())
        );
    }
    ...
    async fn records_broker_api_error_with_scrubbed_message() {
        ...
        assert_eq!(rows[0].1, "find_conversion_order");
        assert_eq!(rows[0].2, "error");
        let error_msg = rows[0].3.as_deref().unwrap_or("");
