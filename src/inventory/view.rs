...
impl InventoryView {
    ...
    pub(crate) fn to_dto(&self) -> st0x_dto::Inventory {
        ...
        st0x_dto::Inventory {
            ...
            snapshot_at: Some(self.last_updated),
        }
    }
}
...
mod tests {
    ...
    fn to_dto_converts_equities_and_usdc() {
        ...
        let view = InventoryView::default()
            ...
            .with_usdc(
                Usdc::new(Decimal::from(10000)),
                Usdc::new(Decimal::from(5000)),
            );
        ...
        assert_eq!(aapl_dto.onchain_available.inner(), Decimal::from(100));
        assert_eq!(aapl_dto.onchain_inflight, FractionalShares::ZERO);
        assert_eq!(aapl_dto.offchain_available.inner(), Decimal::from(50));
        assert_eq!(aapl_dto.offchain_inflight, FractionalShares::ZERO);

        assert_eq!(dto.usdc.onchain_available.inner(), Decimal::from(10000));
        assert_eq!(dto.usdc.onchain_inflight, Usdc::ZERO);
        assert_eq!(dto.usdc.offchain_available.inner(), Decimal::from(5000));
        assert_eq!(dto.usdc.offchain_inflight, Usdc::ZERO);

        assert!(dto.snapshot_at.is_some());
    }
    ...
    fn to_dto_includes_inflight_amounts() {
        ...
        assert_eq!(tsla_dto.onchain_available.inner(), Decimal::from(80));
        assert_eq!(tsla_dto.onchain_inflight.inner(), Decimal::from(20));
        assert_eq!(tsla_dto.offchain_available.inner(), Decimal::from(40));
        assert_eq!(tsla_dto.offchain_inflight.inner(), Decimal::from(10));

        assert_eq!(dto.usdc.onchain_available.inner(), Decimal::from(5000));
        assert_eq!(dto.usdc.onchain_inflight.inner(), Decimal::from(1000));
        assert_eq!(dto.usdc.offchain_available.inner(), Decimal::from(3000));
        assert_eq!(dto.usdc.offchain_inflight.inner(), Decimal::from(500));
    }
    ...
    fn to_dto_handles_uninitialized_venues() {
        ...
        assert_eq!(spy_dto.onchain_available.inner(), Decimal::from(75));
