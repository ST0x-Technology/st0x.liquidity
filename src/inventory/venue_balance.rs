...
mod tests {
    ...
    fn equity_balance(available: i64, inflight: i64) -> VenueBalance<FractionalShares> {
        VenueBalance::new(
            FractionalShares::new(ed(&available.to_string())),
            FractionalShares::new(ed(&inflight.to_string())),
        )
    }
    ...
    fn total_sums_available_and_inflight() {
        ...
        assert_eq!(balance.total().unwrap().inner(), ed("150"));
    }
    ...
    fn move_to_inflight_transfers_from_available() {
        let balance = equity_balance(100, 0);
        let amount = FractionalShares::new(ed("30"));

        let result = balance.move_to_inflight(amount).unwrap();

        assert_eq!(result.available().inner(), ed("70"));
        assert_eq!(result.inflight().inner(), ed("30"));
    }
    ...
    fn confirm_inflight_removes_from_inflight() {
        ...
        assert_eq!(result.available().inner(), ed("100"));
        assert_eq!(result.inflight().inner(), ed("20"));
    }
    ...
    fn cancel_inflight_returns_to_available() {
        ...
        assert_eq!(result.available().inner(), ed("130"));
        assert_eq!(result.inflight().inner(), ed("20"));
    }
    ...
    fn add_available_increases_available() {
        ...
        assert_eq!(result.available().inner(), ed("130"));
        assert_eq!(result.inflight().inner(), ed("50"));
    }
    ...
    fn usdc_balance_works_same_as_equity() {
        ...
        let amount = Usdc(ed("30"));

        let result = balance.move_to_inflight(amount).unwrap();

        assert_eq!(result.available().0, ed("70"));
        assert_eq!(result.inflight().0, ed("30"));
    }

    #[test]
    fn apply_snapshot_skips_when_inflight_nonzero() {
        let balance = equity_balance(90, 10);
        let snapshot_balance = FractionalShares::new(ed("95"));

        let result = balance.apply_snapshot(snapshot_balance);

        // Balance unchanged when inflight is non-zero
        assert_eq!(result.available().inner(), ed("90"));
        assert_eq!(result.inflight().inner(), ed("10"));
    }

    #[test]
    fn apply_snapshot_sets_available_when_inflight_zero() {
        let balance = equity_balance(100, 0);
        let snapshot_balance = FractionalShares::new(ed("75"));

        let result = balance.apply_snapshot(snapshot_balance);

        assert_eq!(result.available().inner(), ed("75"));
        assert_eq!(result.inflight().inner(), ExactDecimal::zero());
    }

    #[test]
    fn apply_snapshot_works_with_usdc_when_inflight_zero() {
        let balance = usdc_balance(1000, 0);
        let snapshot_balance = Usdc(ed("950"));

        let result = balance.apply_snapshot(snapshot_balance);

        assert_eq!(result.available().0, ed("950"));
        assert_eq!(result.inflight().0, ExactDecimal::zero());
    }
    ...
    fn force_apply_snapshot_clears_inflight() {
        ...
        assert_eq!(result.available().inner(), ed("95"));
        assert_eq!(result.inflight().inner(), ExactDecimal::zero());
    }
