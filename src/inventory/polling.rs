    ...
    Exe: Executor,
{
    ...
    pub(crate) async fn poll_source_inner(
        &self,
        ...
        match source {
            ...
            OnchainPollSource::Equity => {
                ...
                let observed_at = self
                    .poll_onchain_equity(&self.snapshot_id, &registry, block_number?, fetched_at)
                    .await?;
            ...
            OnchainPollSource::Usdc => {
                let (fetched_at, block_number) =
                    timestamp_before(self.raindex_service.latest_block_number()).await;
                let observed_at = self
                    .poll_onchain_usdc(&self.snapshot_id, &registry, block_number?, fetched_at)
                    .await?;
                Ok(SourcePollOutcome::ObservedAt(observed_at))
            }
        }
    ...
    async fn poll_onchain_equity(
        ...
        self.persist_snapshot_command(InventorySnapshotCommand::OnchainEquity {
            balances,
            fetched_at,
        ...
        // Stamped only after persistence succeeds, so a failed write never
        // leaves this slot falsely fresh.
        for symbol in symbols {
            self.poll_freshness.observe(
    ...
    async fn poll_onchain_usdc(
        ...
        self.persist_snapshot_command(InventorySnapshotCommand::OnchainUsdc {
            usdc_balance,
            fetched_at,
            block_number: Some(block_number),
        })
    ...
    async fn escalate_cash_divergence(
        ...
        self.persist_snapshot_command(InventorySnapshotCommand::ReconcileOffchainUsd {
            usd_balance_cents,
            gross_usd_cents,
            fetched_at,
    ...
    async fn escalate_divergence(
        ...
        self.persist_snapshot_command(InventorySnapshotCommand::ReconcileOffchainEquity {
    ...
    Exe::Error: std::error::Error + Send + Sync + 'static,
{
    async fn poll_source(
        ...
        Self::poll_source_inner(self, source)
            .await
...
mod tests {
    ...
    async fn offchain_source_does_not_record_freshness_when_executor_returns_unimplemented() {
        ...
        service
            .poll_source_inner(InventoryObservationSource::OffchainInventory)
            .await
            .unwrap();
    ...
    async fn onchain_usdc_source_does_not_depend_on_equity_polling() {
        ...
        let asserter = Asserter::new();
        asserter.push_success(&serde_json::Value::from(100u64)); // eth_blockNumber
        ...
        service
            .poll_source_inner(InventoryObservationSource::OnchainUsdc)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn base_wallet_wrapped_equity_source_stamps_poll_freshness() {
        let pool = setup_test_db().await;
        let provider = mock_provider();
        let (orderbook, order_owner) = test_addresses();
        let symbol = test_symbol("AAPL");
        let raw_balance = U256::from(10u64).pow(U256::from(18u64));
        let asserter = Asserter::new();
        asserter.push_success(&alloy::hex::encode_prefixed(raw_balance.abi_encode()));
        let base_wallet = MockBaseWallet::with_asserter(&asserter);
        let mut wrapped_equity_token_addresses = HashMap::new();
        wrapped_equity_token_addresses.insert(
            symbol.clone(),
        ...
        service
            .poll_source_inner(InventoryObservationSource::BaseWalletWrappedEquity)
            .await
            .unwrap();
