...
pub(crate) enum ConductorSpawnError {
    ...
    #[error(
        "hedged chain {chain} has no {what} wired; the provider, queue, and \
    ...
    #[error("{chain} gas monitor selection and wallet availability disagree")]
    GasWalletMismatch { chain: Chain },
...
pub(crate) struct ConductorCtx<Prov, Exec> {
    ...
    pub(crate) ctx: Ctx,
    /// Providers for hedged, non-primary chains (the primary's fill watcher
...
/// The equity and USDC vaults one hedged chain's assets table configures.
    ...
    ctx.chains
        ...
        .map(|hedged| {
            (
                hedged.chain,
                MarketMakingSlots {
                    equity_symbols: chain_equity_symbols(&hedged.assets),
                    usdc_tracking_enabled: hedged.assets.cash.is_some(),
                },
            )
        })
        .collect()
...
fn configured_chain_vaults(hedged: &st0x_config::HedgedChain) -> ConfiguredChainVaults {
    let mut equity_vaults: BTreeMap<Address, BTreeSet<B256>> = BTreeMap::new();
    for equity_config in hedged.assets.equities.symbols.values() {
        equity_vaults
...
/// The vault-reading leg of the inventory poller, one entry per hedged
    ...
    ctx.chains
        ...
        .map(|hedged| {
            ...
            let ConfiguredChainVaults {
                equity_vaults,
                usdc_vaults,
            } = configured_chain_vaults(hedged);
    ...
    crate::offchain::order::JobError: From<Exec::Error>,
{
    ...
    // One accounting entry per hedged chain: the primary reuses the main
    ...
    for hedged in context.ctx.chains.hedged() {
        let provider = if hedged.chain == context.ctx.chains.primary().chain {
            context.provider.clone()
        } else {
            watch_providers.get(&hedged.chain).cloned().ok_or(
                ConductorSpawnError::MissingWatchWiring {
                    chain: hedged.chain,
    ...
    // One fill watcher per hedged chain, each with its own provider, poll
    ...
    let hedged_chains: Vec<st0x_config::HedgedChain> =
        context.ctx.chains.hedged().cloned().collect();
    for hedged in hedged_chains {
        let chain = hedged.chain;
        let provider = if chain == primary_chain {
            context.provider.clone()
        } else {
        ...
        let poll_interval = hedged.order_fill_poll_interval;
        let monitor =
            OrderFillMonitor::new(hedged, queue, context.pool.clone(), provider, poll_interval);
    ...
    crate::offchain::order::JobError: From<Exec::Error>,
{
    ...
    fn spawn_apalis_monitor(self) -> JoinHandle<Result<(), MonitorTaskError>> {
        ...
        tokio::spawn(startup_token.wrap(async move {
            ...
            // worker per hedged chain, each on its own namespaced queue, so
...
struct GasMonitors {
    ...
    robinhood: Option<GasMonitor>,
}

fn gas_monitor_tasks(
    monitors: Option<GasMonitors>,
    tokens: [StartupToken; 4],
    ...
    let monitors = match monitors {
        Some(GasMonitors {
            ...
            hyperevm,
            robinhood,
        }) => [Some(base), Some(ethereum), hyperevm, robinhood],
...
/// Builds monitors with each chain's own provider, wallet and validated
/// threshold. The hedged secondaries are optional: each is monitored exactly
/// when the alerts config selected it, and the wallet handed in must agree.
fn build_gas_monitors<BaseProv, EthereumProv, HyperProv, RobinhoodProv>(
    alerts: &AlertsCtx,
    base: (BaseProv, Address),
    ethereum: (EthereumProv, Address),
    hyperevm: Option<(HyperProv, Address)>,
    robinhood: Option<(RobinhoodProv, Address)>,
    notifier: &Arc<dyn Notifier>,
...
where
    ...
    RobinhoodProv: Provider + Send + Sync + 'static,
{
    for (chain, wallet_present) in [
        (Chain::HyperEvm, hyperevm.is_some()),
        (Chain::Robinhood, robinhood.is_some()),
    ] {
        if alerts.low_balance_threshold_wei(chain).is_some() != wallet_present {
            return Err(ConductorSpawnError::GasWalletMismatch { chain });
        }
    }
...
mod tests {
    ...
    /// hedged chain needs an entry of its own -- keyed on that chain's
    ...
    #[tokio::test]
    async fn vault_polling_entries_cover_every_hedged_chain() {
        ...
        assert_eq!(
            ...
            "each hedged chain must get its own vault-polling entry, reading through its own \
             provider"
        );
    }

    /// A hedged chain with no provider wired must refuse the spawn. Skipping
    ...
    fn vault_polling_entries_refuse_a_hedged_chain_without_a_provider() {
        ...
        let Err(error) = vault_polling_entries(
            ...
            panic!("a hedged chain with no vault-polling provider must refuse the spawn");
        };
        ...
        assert!(
            ...
            "a hedged chain with no vault-polling provider must refuse the spawn, got {error:?}"
        );
    }

    /// A hedged chain's balances are captured in the daily snapshot, so its
    /// market-making slots must gate the capture too -- with only the assets
    /// that chain's own table declares.
    #[test]
    fn market_making_slots_cover_every_hedged_chain() {
        let secondary_symbol = Symbol::new("NVDA").unwrap();
        let mut ctx = create_test_ctx_with_order_owner(Address::ZERO);
        ctx.chains.primary_mut().assets = ChainAssets {
            equities: ChainEquities {
    ...
    async fn absent_alerts_acknowledge_all_gas_startup_slots_without_tasks() {
        ...
        let names: Vec<_> = gas_monitor_tasks(
            None,
            [
                barrier.token(),
                barrier.token(),
    ...
    #[tokio::test]
    async fn unselected_secondaries_acknowledge_their_slots_without_a_task() {
        let alerts = AlertsCtx::for_test(
            BTreeMap::from([
                (Chain::Base, U256::from(100_u64)),
                (Chain::Ethereum, U256::from(200_u64)),
    ...
    #[tokio::test]
    async fn selected_secondary_tasks_acknowledge_only_their_own_startup_slot_when_polled() {
        let alerts = AlertsCtx::for_test(
            BTreeMap::from([
                (Chain::Base, U256::from(100_u64)),
                (Chain::Ethereum, U256::from(200_u64)),
                (Chain::HyperEvm, U256::from(300_u64)),
                (Chain::Robinhood, U256::from(400_u64)),
            ]),
            Duration::from_secs(300),
            Duration::from_secs(3600),
        );
        let wallets = Chain::ALL.map(|_| {
        ...
        assert_eq!(
            ...
            vec![
                ...
                ("gas-monitor-robinhood", Chain::Robinhood),
            ]
        );
    ...
    #[test]
    fn secondary_monitor_requires_matching_wallet_selection() {
        for chain in [Chain::HyperEvm, Chain::Robinhood] {
            for selected in [false, true] {
                for wallet_present in [false, true] {
                    ...
                    if selected {
                        thresholds.insert(chain, U256::from(300_u64));
                    }
                    let alerts = AlertsCtx::for_test(
                        thresholds,
                        Duration::from_secs(300),
                    ...
                    let (hyperevm, robinhood) = match chain {
                        Chain::HyperEvm => (wallet_present.then(mocked), None),
                        Chain::Robinhood => (None, wallet_present.then(mocked)),
                        Chain::Base | Chain::Ethereum => unreachable!("always monitored"),
                    };
                    ...
                    if selected == wallet_present {
                        ...
                        let selected_monitor = match chain {
                            Chain::HyperEvm => monitors.hyperevm,
                            Chain::Robinhood => monitors.robinhood,
                            Chain::Base | Chain::Ethereum => unreachable!("always monitored"),
                        };
                        assert_eq!(
                            selected_monitor.map(|monitor| monitor.chain),
                            selected.then_some(chain)
                        );
                    } else {
                        assert!(matches!(
                            result,
                            Err(ConductorSpawnError::GasWalletMismatch { chain: mismatched })
                                if mismatched == chain
                        ));
                    }
                }
    ...
    async fn gas_monitors_use_their_chain_provider_wallet_and_threshold() {
        ...
        assert!(
            robinhood.is_none(),
            "no Robinhood threshold selects no monitor"
        );
