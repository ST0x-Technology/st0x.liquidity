    ...
    crate::offchain::order::JobError: From<Exec::Error>,
{
    ...
    // widening the generic `Executor` bound.
    let BrokerCtx::AlpacaBrokerApi(alpaca_ctx) = &context.ctx.broker;
    let asset_eligibility_monitor = Some(AssetEligibilityMonitor {
        broker_ctx: alpaca_ctx.clone(),
        symbols: configured_equity_symbols(&context.ctx)
            .into_iter()
            .collect(),
