...
impl Conductor {
    pub(crate) async fn start<B: Broker + Clone + Send + 'static>(
        ...
        // ClearV3 and TakeOrderV3 events come from IOrderBookV5
        let orderbook_v5 = IOrderBookV5Instance::new(config.evm.orderbook, &provider);

        let mut clear_stream = orderbook_v5.ClearV3_filter().watch().await?.into_stream();
        let mut take_stream = orderbook_v5
            .TakeOrderV3_filter()
            .watch()
            .await?
            .into_stream();
...
mod tests {
    ...
    async fn test_event_enqueued_when_trade_conversion_returns_none() {
        ...
        let clear_event = ClearV3 {
            ...
            alice: crate::test_utils::get_test_order_v4(),
            bob: crate::test_utils::get_test_order_v4(),
    ...
    async fn test_clear_v2_event_filtering_without_errors() {
        ...
        let mut alice_order = crate::test_utils::get_test_order_v4();
        let mut bob_order = crate::test_utils::get_test_order_v4();
