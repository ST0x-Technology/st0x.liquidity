...
async fn run_bot_session_inner(
    ...
    // guard therefore stays armed through coordinated shutdown. The startup
    // error returns below intentionally leave it armed. Once the coordinated
    // shutdown future starts, every return from that future has stopped the
    // conductor and both supervisors, so `finish_session_shutdown` disarms the
    // guard immediately afterward and before propagating the shutdown result.
...
mod tests {
    ...
    #[tokio::test]
    #[tracing_test::traced_test]
    async fn cancelling_pending_coordinated_shutdown_drops_armed_guard() {
        let conductor_task = tokio::spawn(std::future::pending::<()>());
        let conductor_abort = conductor_task.abort_handle();
        let supervisor = pending_test_supervisor();
        ...
        let shutdown_task = tokio::spawn(async move {
            ...
            let coordinated_shutdown = async move {
                shutdown_supervisor(&supervisor_for_shutdown);
                supervisor_for_shutdown.wait().await?;
                shutdown_started_tx
                    .send(())
                    .expect("the test must observe coordinated shutdown becoming pending");
                std::future::pending().await
            };

            finish_session_shutdown(&mut guard, coordinated_shutdown).await
        });

        tokio::time::timeout(Duration::from_secs(1), shutdown_started_rx)
            .await
            .expect("coordinated shutdown must reach its pending window")
            .expect("the shutdown task must report its pending window");
        shutdown_task.abort();
        let cancellation = tokio::time::timeout(Duration::from_secs(1), shutdown_task)
            .await
            .expect("the in-progress shutdown must cancel promptly")
            .expect_err("aborting the in-progress shutdown must cancel its task");

        assert!(
            cancellation.is_cancelled(),
            "the in-progress shutdown helper must be cancelled"
        );
        let conductor_cancellation = tokio::time::timeout(Duration::from_secs(1), conductor_task)
            .await
            .expect("the armed guard must cancel its conductor promptly")
            .expect_err("the armed guard must abort, not complete, its conductor");

        assert!(
            conductor_cancellation.is_cancelled(),
            "cancelling coordinated shutdown must not detach the conductor"
        );
        assert!(
            logs_contain("Supervisor was already stopped"),
            "cancellation while shutdown is pending must drop the still-armed guard"
        );
    }
    ...
    async fn test_run_function_error_propagation() {
        ...
        // Executor init runs before the RPC probe, so an unreachable broker
        // fails there -- a distinct failure path from the unreachable-RPC
        // test above, proving executor-init errors propagate too. Port 1
        // refuses connections immediately.
        ctx.broker = crate::test_utils::mock_alpaca_broker_ctx("http://127.0.0.1:1".to_owned());
        let (pool, apalis_pool) = crate::test_utils::setup_test_pools().await;
        let error = Box::pin(run_conductor_session(
            ctx,
            DatabasePools {
        ...
        assert!(
            format!("{error:#}").contains("HTTP client error"),
            "expected executor-init broker connection failure, got: {error:#}"
        );
    }
