...
use st0x_config::{Chain, ExecutionThreshold, TradingChain};
    ...
    async fn perform(&self, ctx: &AccountantCtx<Node, Exec>) -> Result<Self::Output, Self::Error> {
        ...
        reorged.extend(
            detect_dropped_fill_reorgs(
                ...
                &scan.present,
                tip,
                ctx.ctx.chains.sole_trading().required_confirmations,
            )
            .await?,
        );
    ...
    match position
        .send(
            ...
            PositionCommand::AcknowledgeOnChainFill {
                ...
                price_usdc: trade.price_usdc,
                block_timestamp,
                block_number: Some(reinclusion.number),
            },
        )
        .await
    {
...
mod tests {
    ...
    use st0x_config::{
        ChainAssets, ExecutionThreshold, IngestionCutoff, InventoryAdapters, InventoryMode,
        TradingChain, create_test_ctx_with_order_owner,
    };
