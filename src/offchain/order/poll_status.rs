...
impl PollOrderStatus {
        ...
        match order_state {
            ...
            Failed {
                ...
                warn!(
                    ...
                    %symbol,
                    offchain_order_id = %self.offchain_order_id,
                    "Broker reports Failed with filled shares but no avg_price; rescheduling poll"
                );
            ...
            Cancelled {
                ...
                warn!(
                    ...
                    %symbol,
                offchain_order_id = %self.offchain_order_id,
                "Broker reports Cancelled with filled shares but no avg_price; rescheduling poll"
                );
                reschedule_self(ctx, self.offchain_order_id).await
            }
            ...
            PartiallyFilled {
                ...
                self.record_partial_fill(ctx, symbol, shares_filled, avg_price, partially_filled_at)
                    .await
            }
        ...
        E: Executor + Clone + Send + Sync + 'static,
    {
        let Some(price) = avg_price else {
            warn!(
                ...
                %symbol,
                offchain_order_id = %self.offchain_order_id,
                "Broker reports PartiallyFilled but no avg_price; rescheduling poll without UpdatePartialFill"
            );
            return reschedule_self(ctx, self.offchain_order_id).await;
        };
            ...
            match super::broker_fill_exceeds_local(broker_shares_filled, *local_filled) {
                ...
                Ok(false) => {
                    debug!(
                        ...
                        "Skipping duplicate or stale broker partial fill; rescheduling poll"
                    );
                    return reschedule_self(ctx, self.offchain_order_id).await;
                }
        ...
        E: Executor + Clone + Send + Sync + 'static,
    {
        ...
        if matches!(recovered, OffchainOrder::Cancelling { .. }) {
            ctx.offchain_order_store
                .send(
                    &self.offchain_order_id,
                    OffchainOrderCommand::ConfirmCancellation { cancelled_at },
...
mod tests {
    ...
    /// Asserts that the terminal `Cancelled` order releases the position slot
    /// with no fill and no failure anchor. The classification goes through
    /// `terminal_position_finalization` -- the same shared mapping the
    /// `position_check` finalize sweep and conductor recovery use -- and the
    /// implied command is sent to the real `Position` aggregate. (The sweep
    /// itself lives in `position_check`; this verifies the order state
    /// produced by polling carries everything that sweep needs.)
    async fn assert_position_releases_without_fill<E: Executor + Clone + Send + Sync + 'static>(
        infra: &TestInfra<E>,
        symbol: &Symbol,
    ...
    async fn poll_with_cancelled_broker_confirms_local_cancellation() {
        ...
        assert_position_releases_without_fill(&infra, &symbol, order_id, &order).await;
    }
    ...
    async fn poll_with_cancelled_partial_fill_records_fill_before_confirming_cancellation() {
        ...
        let TerminalPositionFinalization::Complete {
            ...
            unreachable!("asserted Complete above");
        };
        infra
            .position
            .send(
        ...
        assert_eq!(
            position.pending_offchain_order_id, None,
            "Completing the retained fill must release the position slot"
        );
        assert_eq!(
        ...
        // record_partial_fill reschedules a poll before the cancellation is
        // confirmed; that follow-up must observe the terminal state and
        // no-op rather than loop or re-route.
        assert_eq!(
            count_jobs(&infra.pool, poll_order_status_job_type()).await,
            1,
            "Partial-fill recording reschedules exactly one follow-up poll"
        );
        PollOrderStatus {
            offchain_order_id: order_id,
        }
