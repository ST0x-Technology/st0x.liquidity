...
use crate::poll::{
    ...
    spawn_bot, spawn_bot_until_cancelled,
};
...
async fn broker_outage_with_submitted_order_isolated_and_restart_recovers() -> anyhow::Result<()> {
    ...
    let ctx = build_ctx()
        ...
        .broker_url_override(latency.endpoint.clone())
        .call()?;
    let (bot_shutdown, mut bot) = spawn_bot_until_cancelled(ctx);
    ...
    bot_shutdown.cancel();
    tokio::time::timeout(Duration::from_secs(120), bot)
        .await
        .expect("the first bot must finish graceful shutdown before its replacement starts")
        .expect("the first bot task must not panic")
        .expect("the first bot session must shut down cleanly");

    // Restore the broker only after the first bot has stopped, so recovery can
    // only be performed by the replacement session.
    infra
        .broker_service
    ...
    let ctx2 = build_ctx()
        ...
        .broker_url_override(latency.endpoint.clone())
        .call()?;
    let (bot2_shutdown, mut bot2) = spawn_bot_until_cancelled(ctx2);
    ...
    bot2_shutdown.cancel();
    tokio::time::timeout(Duration::from_secs(120), bot2)
        .await
        .expect("the replacement bot must finish graceful shutdown")
        .expect("the replacement bot task must not panic")
        .expect("the replacement bot session must shut down cleanly");
    Ok(())
}
