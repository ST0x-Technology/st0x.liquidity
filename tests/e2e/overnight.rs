...
use serde_json::json;
use st0x_config::{ChainAssets, EquityHedgePolicy, HedgedEquities, HedgingAssets, OperationMode};
...
/// An overnight-enabled hedging policy for every symbol the listing
/// carries, extended hours left disabled.
fn overnight_enabled(assets: &ChainAssets) -> HedgingAssets {
    HedgingAssets {
...
/// An overnight-enabled hedging policy that also enables extended
/// hours, for the session-transition restart where the relaunched bot
/// must re-hedge through the extended chain.
