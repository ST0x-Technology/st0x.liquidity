...
/// Verifies that the inventory poller detects unwrapped equity token
/// balances sitting in the Base wallet (outside Raindex vaults).
///
/// `deploy_equity_vault` mints 1M underlying tokens to the owner and
/// deposits half into the ERC-4626 vault. The remaining 500k underlying
/// tokens stay in the owner's wallet, which is exactly what this test
/// polls for.
///
/// Expected CQRS event:
/// - `InventorySnapshot`: BaseWalletEquity (AAPL = 500000)
#[test_log::test(tokio::test)]
async fn base_wallet_equity_balance_detected_by_inventory_poller() -> anyhow::Result<()> {
    let infra = TestInfra::start(vec![("AAPL", dec!(150))], vec![]).await?;
    ...
    crate::poll::poll_for_events(
        &mut bot,
        &infra.db_path,
        "InventorySnapshotEvent::BaseWalletEquity",
        1,
    )
    .await;
    ...
    let equity_event = events
        ...
        .expect("Expected BaseWalletEquity event");

    let balances = equity_event
        .payload
        .get("BaseWalletEquity")
        .and_then(|val| val.get("balances"))
        .and_then(|val| val.as_object())
        .expect("BaseWalletEquity payload missing balances");

    let aapl_balance_str = balances
        .get("AAPL")
        .and_then(|val| val.as_str())
        .expect("BaseWalletEquity balances missing AAPL");
    let aapl_balance: Decimal = aapl_balance_str.parse()?;

    assert_eq!(
        aapl_balance,
        dec!(500000),
        "Base wallet AAPL balance should be 500k (half of 1M supply)"
    );
