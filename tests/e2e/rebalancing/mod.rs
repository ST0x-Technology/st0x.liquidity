...
async fn unwrapped_equity_in_bot_wallet_recovers_into_raindex() -> anyhow::Result<()> {
    ...
    let ctx = build_rebalancing_ctx()
        ...
        // that fires a Base->Alpaca transfer. That transfer holds the per-symbol
        ...
        // rebalancing interaction itself is RAI-90's integration concern.
    ...
    let pool = connect_db(&infra.db_path).await?;
    let recovery_events = fetch_events_by_type(&pool, "UnwrappedEquityRecovery").await?;
    pool.close().await;
