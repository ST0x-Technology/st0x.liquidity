...
- **`.on_event(recovering_circuit_event)`** — observes terminal `Event::Error`
  ...
  transition, and sends an operator alert when alerting is configured. After the
  configured `[worker_circuit].recovery_timeout_secs`, a scheduled task closes
  the circuit and resumes the same worker without restarting the process. It
  records `Recovered` when resume succeeds or the worker is already running, and
  `RecoveryFailed` otherwise. A success resets the consecutive-failure count.
...
4. A structured `opened` transition and, when alerting is configured, an
   operator alert identify the worker and failure
5. The conductor and sibling workers remain running
6. After the configured timeout, the scheduled recovery attempts to resume the
   worker and emits a structured `recovered` or `recovery_failed` transition
