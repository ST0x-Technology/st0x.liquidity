...
The baseline query records the pre-change `last_processed_block`. If the cutover
is abandoned after this rewind, the rollback restores that exact value; a
rollback that only restores ownership and config would leave the checkpoint at
the replay start.
...
that start (`Startup token approvals deferred` warning): wait for that send,
then restart so the revokes run before this check. Wait for startup and every
revocation receipt to finish before checking the final onchain allowance values.
...
Every result must be zero. The bot treats a revocation failure as non-fatal at
startup, but it leaves a live capital-exposure surface. Treat any nonzero result
as a hard stop: stop the bot, keep the market quiesced, revoke the stale
allowance manually, and re-run this check until every result reads zero. Do not
proceed with the cutover while any spending authorization is unresolved.
...
5. Revoke `OPERATOR_ROLE` from the bot EOA and from every hook on the retired
   inventory, then verify onchain that each grant reads false. Retain a grant
   only as a reviewed and recorded exception.
6. Restore the legacy config values and vault IDs.
7. If Path B rewound the checkpoint, restore the `last_processed_block` recorded
   in the baseline query and read the row back.
8. Deploy the legacy config.
9. Verify balances, registry ownership, allowances, and fill matching.
