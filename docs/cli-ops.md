...
- `--kind usdc` drives a post-burn `DepositFailed` USDC rebalance to the
  clearing terminal `Reconciled` state, releasing the rebalancing guard. Its
  `--reason` must be one of `funds-moved-manually` or
  `deposit-credited-offline`; any other value is rejected. Valid only from a
  post-burn terminal failure.
