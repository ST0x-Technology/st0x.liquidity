...
owner's_ perspective: how many output tokens per input token. When we take the
...
- **Scenario A** (user sells wtAAPL for USDC): The order's output token is
  wtAAPL, input token is USDC. The `io_ratio` = wtAAPL per USDC (from the
  owner's view). From our perspective: we send USDC (their input) and receive
...
- **Scenario B** (user buys wtAAPL with USDC): The order's output token is USDC,
  input token is wtAAPL. The `io_ratio` = USDC per wtAAPL (from the owner's
  view). From our perspective: we send wtAAPL (their input) and receive USDC
  (their output). Our effective sell price in USDC per wtAAPL = `io_ratio`.
...
Scenario A (we buy wtAAPL cheap, sell AAPL at market):
  our_buy_price = 1 / io_ratio   (USDC per wtAAPL)
...
Scenario B (we buy AAPL at market, sell wtAAPL at order price):
  our_sell_price = io_ratio   (USDC per wtAAPL)
