...
export const layoutRebalanceBars = (
  ...
  const rows = operations.slice(0, options.maxRows).map((operation) => {
    ...
    return {
      ...
      // The backend's first-event-to-completion elapsed time is the truth;
      // summing completed stages would understate operations with a stage
      // still open.
      totalMs:
        operation.totalMs ?? durations.reduce((sum, segment) => sum + segment.ms, 0),
      durations,
    }
  })
