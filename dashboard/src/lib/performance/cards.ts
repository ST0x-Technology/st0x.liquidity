...
export const errorsCard = (
  ...
  return {
    ...
    secondary: `${String(warnings)} warnings · ${String(failures)} lifecycle failures`,
    status:
      failures > 0
        ? 'critical'
        : worstStatus([
...
export const openExposureCard = (
  ...
  const oldest = exposures.reduce((candidate, exposure) =>
    exposure.oldestFillBlockTimestamp < candidate.oldestFillBlockTimestamp
      ? exposure
      : candidate,
  )
  const ageMs = now.getTime() - new Date(oldest.oldestFillBlockTimestamp).getTime()
