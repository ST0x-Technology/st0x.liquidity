...
const BLOCK_READ_CONCURRENCY = 8;
...
export async function readConfirmedBlock<T extends { hash: Hash | null }>(
  operation: () => Promise<T>,
  blockNumber: bigint,
  ...
  for (const delayMs of retryDelaysMs) {
    if (delayMs > 0) await Bun.sleep(delayMs);
    try {
      const block = await operation();
      if (block.hash) return block;
      lastError = new Error(`confirmed block ${blockNumber} has no hash`);
    } catch (error) {
  ...
  throw new BaseRpcError(
    `confirmed block ${blockNumber} was unavailable after ${retryDelaysMs.length} attempts`,
    { cause: lastError },
  );
}

async function readConfirmedBlocks<T extends { hash: Hash | null }>(
  blockNumbers: readonly bigint[],
  operation: (blockNumber: bigint) => Promise<T>,
): Promise<T[]> {
  const blocks = new Array<T>(blockNumbers.length);
  let nextIndex = 0;
  const workers = Array.from(
    { length: Math.min(BLOCK_READ_CONCURRENCY, blockNumbers.length) },
    async () => {
      while (nextIndex < blockNumbers.length) {
        const index = nextIndex;
        nextIndex += 1;
        const blockNumber = blockNumbers[index];
        if (blockNumber === undefined) return;
        blocks[index] = await readConfirmedBlock(
...
async function scanInboxLocked(
  ...
  while (BigInt(checkpoint.nextBlock) <= lastConfirmed) {
    ...
    const blocks = await readConfirmedBlocks(blockNumbers, (blockNumber) =>
      client.getBlock({ blockNumber, includeTransactions: true }),
    );
    for (const [index, block] of blocks.entries()) {
      const number = blockNumbers[index];
      if (number === undefined) {
        throw new Error("confirmed block range returned an unexpected result");
      }
