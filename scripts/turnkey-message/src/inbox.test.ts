...
describe("inbox classification", () => {
  ...
  test("accepts calldata at the exact message limit", () => {
    const message = transactionToMessage(
      {
        blockHash: `0x${"11".repeat(32)}`,
        blockNumber: 10n,
      ...
      async getBlock({ blockNumber }: { blockNumber: bigint }) {
        ...
        return {
          hash:
            blockNumber === 11n
              ? (`0x${"11".repeat(32)}` as Hash)
              : (`0x${blockNumber.toString(16).padStart(64, "0")}` as Hash),
        };
      },
      async getBlockNumber() {
        return 0n;
...
describe("inbox scan boundary", () => {
  test("reads each chunk concurrently within a fixed bound", async () => {
    const directory = await mkdtemp(join(tmpdir(), "turnkey-inbox-pool-"));
    let active = 0;
    let maximumActive = 0;
    ...
    const client = {
      ...
      async getBlock({ blockNumber }: { blockNumber: bigint }) {
        ...
        await Bun.sleep(Number(20n - blockNumber));
        active -= 1;
    ...
    expect(completed).not.toEqual(completed.toSorted((a, b) => Number(a - b)));
    expect(result.checkpoint.recentBlocks.map(({ number }) => number)).toEqual(
      Array.from({ length: 20 }, (_, number) => number.toString()),
    );
  });
  ...
  test("recovers when a confirmed block appears on the third attempt", async () => {
    ...
    const client = {
      ...
      async getBlock() {
        blockCalls += 1;
        if (blockCalls < 3) {
          throw new BlockNotFoundError({ blockNumber: 7n });
        }
        return {
          hash: blockHash,
          transactions: [
  ...
  test("raises BaseRpcError after confirmed block reads stay hashless", async () => {
    let calls = 0;
  ...
  test("does not retry unrelated confirmed block errors", async () => {
