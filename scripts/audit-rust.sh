...
actual_ignored_advisories="$(
  sed -n '/^ignore = \[/,/^\]$/p' .cargo/audit.toml |
    grep -Eo 'RUSTSEC-[0-9]{4}-[0-9]{4}' || true
)"
actual_ignored_advisories="$(printf '%s\n' "$actual_ignored_advisories" | sort -u)"

if [[ "$actual_ignored_advisories" != "$expected_ignored_advisories" ]]; then
  printf 'Unexpected ignored advisories\nExpected:\n%s\nActual:\n%s\n' \
    "$expected_ignored_advisories" "$actual_ignored_advisories" >&2
  exit 1
fi
