...
- **CRITICAL: ALL workspace checks must pass before work is considered done**
  (except for documentation-only changes that modify only markdown or docs
  files). Do not assume any failure is "pre-existing" -- every warning, error,
  and test failure must be fixed regardless of origin. CI runs full workspace
  checks and will block PR merging on any failure.
