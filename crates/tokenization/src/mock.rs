...
mod tests {
    use super::{
        MockTokenizer, TokenizationRequest, TokenizationRequestStatus, TokenizationRequestType,
        Tokenizer,
    };
    use crate::{ClientRequestId, issuer_request_id, tokenization_request_id};

    #[tokio::test]
    async fn mint_lookup_excludes_non_mint_history_with_matching_ids() {
        let id = issuer_request_id("mint-history");
        let mut mint = TokenizationRequest::mock(TokenizationRequestStatus::Pending);
        mint.r#type = Some(TokenizationRequestType::Mint);
        mint.id = tokenization_request_id("matching-mint");
        mint.client_request_id = Some(ClientRequestId::from(&id));
        let mut redemption = mint.clone();
        redemption.r#type = Some(TokenizationRequestType::Redeem);
        let mut unknown = mint.clone();
        unknown.r#type = None;
        let mut other_mint = mint.clone();
        other_mint.client_request_id = Some(ClientRequestId::from(&issuer_request_id("other")));
        let unrelated = vec![redemption, unknown, other_mint];
        let tokenizer = MockTokenizer::new().with_pending_requests(unrelated.clone());
        assert!(
            tokenizer
                .find_mint_by_issuer_request_id(&id)
                .await
                .unwrap()
                .is_none()
        );

        let mut history = unrelated;
        history.push(mint.clone());
        let tokenizer = MockTokenizer::new().with_pending_requests(history);
        let found = tokenizer
            .find_mint_by_issuer_request_id(&id)
            .await
            .unwrap()
        ...
        assert_eq!(found.r#type, Some(TokenizationRequestType::Mint));
    }
