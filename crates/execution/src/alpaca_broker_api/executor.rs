...
pub(super) struct AssetResponse {
    /// The asset the broker answered with. Compared against the requested
    /// symbol before the attributes are cached or snapshotted, so another
    /// asset's payload can never authorize this one.
    pub symbol: Symbol,
