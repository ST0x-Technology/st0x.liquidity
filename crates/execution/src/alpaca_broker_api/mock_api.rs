...
mod tests {
    ...
    use crate::alpaca_broker_api::{AlpacaBrokerApi, TimeInForce};
    use crate::{Executor, MarketSession, Symbol};
    use st0x_float_macro::float;
    ...
    async fn market_hours_parse_the_mock_calendar() {
        ...
        assert!(executor.is_market_open().await.unwrap());
        assert_eq!(
            executor.market_session().await.unwrap(),
