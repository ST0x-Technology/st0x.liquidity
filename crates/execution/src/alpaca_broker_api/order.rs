...
use crate::{
    ALPACA_MAX_DECIMAL_PLACES, AlpacaAmount, CancellationOutcome, ClientOrderId, Direction,
    ExecutorOrderId, FractionalShares, MarketOrder, OrderFailureTerminality, OrderPlacement,
    OrderStatus, OrderUpdate, Permanence, Positive, Symbol, Usd, Usdc,
...
/// Alpaca's overnight venue rejects a new sell with a plain 422 while an
/// earlier closing order for the position is still open. Distinguished from
/// every other 422 by the message, exactly like the duplicate-id case below,
/// so the two are never confused: that one adopts, this one retries.
fn is_consecutive_sell_pending(error: &AlpacaBrokerApiError) -> bool {
    matches!(
        error,
...
pub(super) async fn place_limit_order(
        ...
        .await
    {
        ...
        // which adopts.
        Err(error) if is_consecutive_sell_pending(&error) => {
            return Err(AlpacaBrokerApiError::ConsecutiveSellPending {
...
mod tests {
    ...
    #[test]
    fn overnight_order_refuses_another_symbols_snapshot() {
        ...
        assert!(
            matches!(
                ...
                ) if *requested == Symbol::new("RKLB").unwrap()
                    && *snapshot_symbol == Symbol::new("AAPL").unwrap()
            ),
    ...
    fn overnight_order_rejects_an_over_precise_quantity_instead_of_truncating() {
        ...
        // The offending quantity is asserted as a typed field rather than
        // through the rendered message: an operator needs to see which
        // quantity was refused, but the prose around it is free to change.
        assert!(
            matches!(
                &error,
                ...
                } if *shares == overnight_shares("0.1234567891")
            ),
            "expected QuantityTooPrecise naming the offending quantity, got {error:?}"
        );
        assert_eq!(error.permanence(), Permanence::Permanent);
    }
    ...
    async fn place_overnight_order_classifies_a_consecutive_sell_rejection_as_transient() {
        ...
        let AlpacaBrokerApiError::ConsecutiveSellPending { source } = &error else {
            panic!("expected ConsecutiveSellPending, got {error:?}");
        };
        // The original API error is kept whole, not flattened to a string:
        // its status and body still describe the rejection.
        let AlpacaBrokerApiError::ApiError {
            status, message, ..
        } = source.as_ref()
        else {
            panic!("expected the wrapped ApiError, got {source:?}");
        };
        assert_eq!(*status, StatusCode::UNPROCESSABLE_ENTITY);
        assert!(
            message.contains("open closing position orders"),
            "the wrapped error should carry the broker's reason, got {message:?}"
        );
