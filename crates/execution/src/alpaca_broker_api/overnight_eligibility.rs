...
impl EligibilitySnapshots {
    ...
    /// Records a snapshot under the symbol it carries, so the map key and
    /// the snapshot's identity cannot diverge. Private on purpose: the
    /// only production writer is [`sync_eligibility`], and a public
    /// mutation path would let a consumer report an arbitrary snapshot as
    /// broker state.
    fn record(&self, snapshot: EligibilitySnapshot) {
        self.inner
            .write()
            .unwrap_or_else(PoisonError::into_inner)
    ...
    pub fn seed_for_test(&self, snapshot: EligibilitySnapshot) {
...
pub enum OvernightEligibilityError {
    ...
    #[error(
        "eligibility snapshot for {symbol} claims a future sync time ({synced_at}); \
         refusing to place from an untrustworthy clock"
    )]
    SnapshotFromFuture {
        symbol: Symbol,
        synced_at: DateTime<Utc>,
...
pub fn validate_overnight_eligibility(
    ...
    // A snapshot stamped after `now` means the clock stepped backward
    // since the sync: the snapshot may belong to a later session than
    // the one being validated, so it authorizes nothing (fail closed).
    if snapshot.synced_at > now {
        return Err(OvernightEligibilityError::SnapshotFromFuture {
            symbol: symbol.clone(),
            synced_at: snapshot.synced_at,
        });
    }
...
mod tests {
    ...
    #[test]
    fn snapshot_from_the_future_fails_closed() {
        // Synced "after" now: only a backward clock step produces this.
        // Such a snapshot may belong to a later session, so it must not
        // authorize this one.
        let mut snapshot = eligible_snapshot();
        snapshot.synced_at = now() + chrono::Duration::hours(1);

        let error = validate(Some(&snapshot), OvernightOrderShape::WholeShares).unwrap_err();
        assert!(
            matches!(
                error,
                OvernightEligibilityError::SnapshotFromFuture { ref symbol, synced_at }
                    if *symbol == rklb() && synced_at == snapshot.synced_at
            ),
            "expected SnapshotFromFuture, got {error:?}"
        );
    }
    ...
    fn mid_session_startup_sync_authorizes_the_running_session() {
        ...
        // inside the session's window and authorizes placements from
        // that moment on.
        let mut snapshot = eligible_snapshot();
        ...
        validate_overnight_eligibility(
            ...
            Some(&snapshot),
            OvernightOrderShape::WholeShares,
            Utc.with_ymd_and_hms(2026, 8, 29, 2, 45, 0).unwrap(),
        )
        .unwrap();
    }
