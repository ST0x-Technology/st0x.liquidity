...
pub enum AlpacaMarketDataError {
    ...
    #[error(
        "overnight quote timestamp is {:.3}s ahead of local time, beyond the allowed skew of \
         {}s; refusing to price from an untrustworthy timestamp",
        ahead.as_secs_f64(),
        max_age.as_secs()
    )]
    QuoteFromFuture { ahead: Duration, max_age: Duration },
...
/// timestamp slightly ahead of `now` (clock skew) counts as age zero,
/// but only within the same `max_age` bound: a stamp further ahead is
/// not skew but an untrustworthy clock, and would otherwise stay
/// "fresh" until local time catches up, so it is refused (fail closed).
pub fn validate_overnight_quote_age(
        ...
        && ahead > max_age
    {
        return Err(AlpacaBrokerApiError::LatestQuote(Box::new(
            AlpacaMarketDataError::QuoteFromFuture { ahead, max_age },
        )));
    }
...
mod tests {
    ...
    #[test]
    fn quote_stamped_far_in_the_future_is_rejected() {
        ...
        assert!(
            matches!(
                ...
                AlpacaBrokerApiError::LatestQuote(inner) if matches!(
                    **inner,
                    AlpacaMarketDataError::QuoteFromFuture { ahead, max_age }
                        if ahead == Duration::from_secs(120) && max_age == Duration::from_secs(30)
                )
            ),
            "expected QuoteFromFuture, got {error:?}"
        );
    }
