...
sol!(
    ...
    IOrderBookV5,
    "../../lib/rain.orderbook/out/IOrderBookV5.sol/IOrderBookV5.json"
);
...
///
/// ```ignore
/// let service = Self::new(provider, orderbook_address);
///
...
where
    P: Provider + Clone,
{
    pub fn new(provider: P, orderbook: Address) -> Self {
        Self {
    ...
    P: Provider + Clone + Send + Sync + 'static,
{
    ...
    async fn deposit_usdc(&self, vault_id: VaultId, amount: U256) -> Result<TxHash, Self::Error> {
        Self::deposit_usdc(self, vault_id, amount).await
    }
    ...
    async fn withdraw_usdc(
        ...
        Self::withdraw_usdc(self, vault_id, target_amount).await
    }
}
...
mod tests {
    ...
    use super::{IOrderBookV5, VaultError, VaultService};
    use crate::VaultId;
    ...
    sol!(
        ...
        #[derive(serde::Serialize, serde::Deserialize)]
        TOFUTokenDecimals,
        "../../lib/rain.orderbook/out/TOFUTokenDecimals.sol/TOFUTokenDecimals.json"
    );
    ...
    enum LocalEvmError {
        ...
        InvalidPrivateKey(#[source] signers::Error),
        #[error("Contract error")]
        Contract(#[from] contract::Error),
        #[error("Provider error")]
        Provider(#[from] PendingTransactionError),
        #[error("RPC error")]
