...
/// credentials, the wallet address, and a base provider. The main
/// crate assembles this from its own config/secrets split.
...
impl<P: Provider + Clone + Send + Sync + 'static> TurnkeyWallet<P> {
    ...
    pub async fn new(ctx: TurnkeyCtx<P>) -> Result<Self, EvmError> {
        ...
        let signer = TurnkeySigner::from_api_key(
            ctx.api_private_key.as_str(),
            ctx.organization_id.into_inner(),
            ctx.address,
            Some(chain_id),
        )?;
