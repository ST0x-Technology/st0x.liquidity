...
mod tests {
    ...
    async fn send_lock_holds_through_the_whole_nonce_too_low_retry_loop() {
        ...
        let submit_delay = Duration::from_millis(50);
        ...
        assert_eq!(
            elapsed,
            NONCE_RETRY_BACKOFF + submit_delay * 3,
            "the first call's backoff sleep must actually elapse under the \
             paused clock with the configured submit delay"
        );
    ...
    fn release_in_flight_after_wait_releases_whole_nonce_on_confirm() {
        ...
        in_flight.record(WALLET, STUCK_NONCE, original_hash);
        in_flight.record(WALLET, STUCK_NONCE, confirmed_replacement_hash);
        ...
        assert_eq!(
            in_flight.ownership(WALLET, STUCK_NONCE),
            NonceOwnership::Unknown,
            "a mined replacement makes every competing hash at its nonce \
             obsolete, so the whole nonce entry must be released"
        );
    }
    ...
    fn release_in_flight_after_wait_releases_only_dropped_hash() {
        ...
        in_flight.record(WALLET, STUCK_NONCE, replacement_hash);
        ...
        assert_eq!(
            in_flight.ownership(WALLET, STUCK_NONCE),
            NonceOwnership::Ours,
            "a dropped attempt is definitive only for that hash; its \
             replacement at the same nonce must remain in flight"
        );
    }
