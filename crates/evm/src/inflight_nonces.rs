...
//! `await_receipt` releases it once the wait resolves definitively. A mined
//! transaction clears every competing hash recorded at its nonce, while a
//! transaction proven dropped from both the receipt lookup and the mempool
//! clears only that hash. This lets a later "replacement transaction
//! underpriced" rejection be checked against direct proof of ownership
//! instead of an inferred heuristic.
//!
...
impl InFlightNonces {
    ...
    pub(crate) fn release_nonce_for_hash(&self, address: Address, tx_hash: TxHash) {
        ...
        record
            .per_nonce
            .retain(|_nonce, tx_hashes| !tx_hashes.contains_key(&tx_hash));
    }
