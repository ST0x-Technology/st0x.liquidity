...
impl AlpacaBrokerMock {
    ...
    /// Returns the Alpaca deposit address from mock state.
    pub fn deposit_address(&self) -> Address {
        let state = lock(&self.state);
        state
            .alpaca_deposit_address
            .parse()
            .expect("deposit address should be valid")
    }
                ...
                for block_num in (last_block + 1)..=current {
                    ...
                    for tx_hash in block.transactions.hashes() {
                        ...
                        for log in receipt.inner.logs() {
                            ...
                            let mut mock_state = lock(&state);
                            let already_exists = mock_state
                                .wallet_transfers
                                .iter()
                            ...
                            let amount_raw: u64 = event.value.try_into().unwrap_or(0);
                            let amount_usdc =
                                Decimal::from(amount_raw) / Decimal::from(1_000_000u64);

                            mock_state.wallet_transfers.push(MockWalletTransfer {
            ...
            let response_body = {
                ...
                match mode {
                    MockMode::HappyPath => {
                        ...
                        // Check per-symbol delay before filling
                        let delay = state
                            .symbol_fill_delays
                            .get(&symbol_key)
                            .copied()
                        ...
                        if poll_count >= delay {
                            let fill_price =
                                state.symbol_fill_prices.get(&symbol_key).copied().unwrap();
