...
mod tests {
    ...
    fn deserialize_backward_compat_decimal_strings() {
        ...
        for json in test_cases {
            let _: ExactDecimal = serde_json::from_str(json).unwrap_or_else(|error| {
                panic!("Failed to deserialize backward-compat string {json}: {error}")
            });
        }
    }
    ...
    fn deserialize_json_float() {
        let deserialized: ExactDecimal = serde_json::from_str("100.0").unwrap();
        assert_eq!(deserialized, ExactDecimal::parse("100").unwrap());

        let deserialized: ExactDecimal = serde_json::from_str("22.7299").unwrap();
        assert_eq!(deserialized, ExactDecimal::parse("22.7299").unwrap());
    }

    #[test]
    fn deserialize_json_integer() {
        let deserialized: ExactDecimal = serde_json::from_str("42").unwrap();
        assert_eq!(deserialized, ExactDecimal::parse("42").unwrap());

        let deserialized: ExactDecimal = serde_json::from_str("0").unwrap();
        assert_eq!(deserialized, ExactDecimal::parse("0").unwrap());
    }

    #[test]
    fn deserialize_json_negative_integer() {
        let deserialized: ExactDecimal = serde_json::from_str("-5").unwrap();
        assert_eq!(deserialized, ExactDecimal::parse("-5").unwrap());
    }

    #[test]
    fn deserialize_optional_json_number() {
        #[derive(Deserialize)]
        struct Wrapper {
            #[serde(default)]
        ...
        assert_eq!(parsed.value, Some(ExactDecimal::parse("100").unwrap()));
