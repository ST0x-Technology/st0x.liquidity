...
fn is_bot_resumable_wait(error: &UsdcTransferError) -> bool {
    match error {
        ...
        | UsdcTransferError::InventoryView(_)
        | UsdcTransferError::WithdrawableCashUnavailable { .. }
...
pub(crate) async fn recheck_transfer_command<W: Write>(
    ...
    let transfer_kind = match transfer_type {
        ...
        RecheckTransferType::Redemption => {
            st0x_hedge::operator::equity_transfer::RecheckKind::Redemption
        }
...
mod tests {
        ...
        let mock = server
            .mock_async(|when, then| {
                ...
                then.status(200).body(format!(
                    r#"{{"outcome":"{}"}}"#,
                    st0x_hedge::api::USDC_RESUME_ENQUEUED_OUTCOME
                ));
            })
            .await;
    ...
    async fn reconcile_usdc_transfer_succeeds_from_alpaca_to_base_no_burn_bridging_failed() {
        ...
        let (store, _projection) = StoreBuilder::<UsdcRebalance>::new(pool.clone())
            ...
            .await
            .unwrap();
        // Seed the production AlpacaToBase history: the conversion leg runs
        // first, and `Initiate` (the withdrawal intent) is only valid from
        // `ConversionComplete` for this direction.
