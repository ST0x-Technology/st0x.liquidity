...
fn classify_command(command: Commands) -> anyhow::Result<CommandRoute> {
    Ok(match command {
        ...
        Commands::MarketSession => CommandRoute::Simple(SimpleCommand::MarketSession),
        Commands::Quote { symbol, feed } => {
            CommandRoute::Simple(SimpleCommand::Quote { symbol, feed })
        }
        Commands::Asset { symbol } => CommandRoute::Simple(SimpleCommand::Asset { symbol }),
...
mod tests {
    ...
    fn classify_command_routes_market_session_as_simple() {
        let Ok(CommandRoute::Simple(SimpleCommand::MarketSession)) =
            classify_command(Commands::MarketSession)
        else {
    ...
    fn classify_command_routes_quote_as_simple() {
        ...
        let Ok(CommandRoute::Simple(SimpleCommand::Quote { symbol, feed })) = classified else {
    ...
    fn classify_command_routes_asset_as_simple() {
        ...
        let Ok(CommandRoute::Simple(SimpleCommand::Asset { symbol })) = classified else {
    ...
    fn classification_preserves_the_client_order_id_for_buy_and_sell() {
        ...
        let Ok(CommandRoute::Simple(SimpleCommand::Buy {
        ...
        let Ok(CommandRoute::Simple(SimpleCommand::Sell {
    ...
    async fn run_command_with_writers_executes_buy() {
        ...
        let mut ctx = create_test_ctx();
        ctx.broker =
            st0x_hedge::operator::test_utils::mock_alpaca_broker_ctx(broker_mock.base_url());
        let pool = setup_test_db().await;
        let command = Commands::Buy {
            symbol: Symbol::new("AAPL").unwrap(),
            quantity: positive_shares("1"),
