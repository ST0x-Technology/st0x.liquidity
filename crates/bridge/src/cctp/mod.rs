...
impl<EthWallet: Wallet, BaseWallet: Wallet> CctpBridge<EthWallet, BaseWallet> {
    ...
    /// `amount` at or after `from_block`, for crash-safe resume. Delegates to the
    /// source endpoint for the given direction. See
    /// [`CctpEndpoint::find_recent_burn`].
    pub async fn find_recent_burn(
        &self,
        direction: BridgeDirection,
        amount: U256,
        from_block: u64,
