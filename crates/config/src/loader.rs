...
mod tests {
    ...
    async fn log_query_url_template_without_id_placeholder_fails() {
        let config = toml_file(&format!(
            r#"log_query_url_template = "https://logs.example/query?id=missing"{MINIMAL_CONFIG_CORE}{TOKENIZATION_SECTION}{BOT_GAS_VALUATION_SECTION}{REBALANCING_SECTION}"#
        ));
        let secrets = alpaca_secrets_toml();
    ...
    async fn log_query_url_template_that_is_not_a_url_fails() {
        let config = toml_file(&format!(
            r#"log_query_url_template = "not a url {{id}}"{MINIMAL_CONFIG_CORE}{TOKENIZATION_SECTION}{BOT_GAS_VALUATION_SECTION}{REBALANCING_SECTION}"#
        ));
        let secrets = alpaca_secrets_toml();
    ...
    fn broker_type_tag_uses_kebab_case() {
        let variants = [("alpaca-broker-api", "AlpacaBrokerApi")];
