...
pub struct Ctx {
    ...
    /// Gas-balance alerting context. Always `Some` on a loaded context: the
    /// `[alerts]` config section is required, and its absence fails startup
    /// ([`crate::alerts::AlertsAssemblyError::MissingConfig`]). `None` only
    /// in test-support contexts built without a gas monitor.
...
mod tests {
    ...
    /// The `[alerts]` section is required: a bot without a gas monitor
    /// silently stops transacting when its wallet runs dry, so an omitted
    /// section must fail startup rather than skip the monitor.
    #[tokio::test]
    async fn startup_fails_when_alerts_section_omitted() {
        let config = toml_file(
            r#"
            database_url = ":memory:"
            server_port = 8080
        ...
        assert!(
            matches!(
                error,
                CtxError::Alerts(crate::alerts::AlertsAssemblyError::MissingConfig)
            ),
            "expected MissingConfig for an omitted [alerts] section, got: {error}"
        );
    }
    ...
    /// `[alerts]` is required at startup
    /// (`AlertsAssemblyError::MissingConfig`), but
    /// `all_repo_config_tomls_are_valid` only exercises the structural parse,
    /// where the section is still `Option` -- so a checked-in config that
    /// dropped it would stay green there and only fail at the
    /// `validate-config` deploy gate or at bot startup. Guard it here, the
    /// same way `repo_config_rebalancing_requires_bot_gas_valuation` guards
    /// its invariant.
    #[test]
    fn repo_config_requires_alerts() {
        for path in repo_config_paths() {
            let contents = std::fs::read_to_string(&path).unwrap();
            let config: Config = toml::from_str(&contents).unwrap();

            assert!(
                config.alerts.is_some(),
                "{path:?}: the [alerts] section is missing -- parse_and_validate \
                 rejects its absence at startup"
            );
        }
    }
