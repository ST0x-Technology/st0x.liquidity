...
global `counter_trade_submission_lock`. The in-process Tokio mutex is paired
with a kernel-managed advisory lock file beside the SQLite database, so daemon
tasks and separate operator CLI processes participate in the same account-wide
critical section. The kernel releases the file lock on process exit, avoiding a
stale lease after a crash. The preflight subtracts buying power already reserved
by live bot buy orders from the current Alpaca cash snapshot. The reservation
total is passed explicitly through the `Executor` and `OrderPlacer` preflight
contracts; the execution crate does not read application projections.
...
`Pending` also retains the exact client order ID, requested quantity, session,
limit price, and close-flatten flag. Any crash recovery replays those durable
terms rather than repricing or changing the order kind from the current market
session. This matters when a crash happens before the broker receives the
request, because client-order deduplication cannot recover an order that was
never submitted.
...
- Concurrent bot hedge attempts cannot reuse cash committed to another live buy,
  including across process restarts and between the daemon and operator CLI.
