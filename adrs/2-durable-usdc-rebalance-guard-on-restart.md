# ADR 2: Reconstruct the USDC rebalancing guard from persisted state on startup
...
- `usdc_in_progress` is `AtomicBool::new(false)` at construction
  (`src/rebalancing/trigger/mod.rs:682`) and is never rebuilt from persisted
  events.
...
rebalance, because USDC bridges have no resume path: `api.rs:1394` ("recheck is
not supported for USDC bridges") and `UsdcRebalance::Materialized = Nil` mean no
poller or reactor catch-up drives an interrupted bridge forward after restart.
