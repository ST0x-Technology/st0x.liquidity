# ADR 0021: Reconstruct pre-burn Alpaca-to-Base USDC inventory before resume

- Status: Proposed
- Date: 2026-07-16
- Issue: RAI-1324
- Amends: ADR 0003 (pre-burn Alpaca-to-Base inventory reconstruction only)
...
- `Withdrawing` or `WithdrawalComplete`: `Initiated` was committed before the
  ...
  remains zero for the whole resumed transfer. Terminal settlement finds no
  tracking entry, returns `DeferredToSnapshot`, and skips the destination
  credit.
...
### Base-to-Alpaca scope

The same pre-burn safety argument applies in principle to Base-to-Alpaca, but
that direction has a different source venue and observation contract. Startup
currently re-arms `WithdrawalSubmitting { BaseToAlpaca }` and pre-burn
`BridgingSubmitting { BaseToAlpaca }` against the MarketMaking source without a
fresh-Hedging-snapshot barrier. This ADR does not claim that accounting window
is reconstructed. It remains guard-only under ADR 0003 until a separate decision
defines the MarketMaking freshness proof, resumable state matrix, and
replacement-based source reservation.
...
- `ConversionComplete`, `Withdrawing`, `WithdrawalComplete`, and Alpaca-to-Base
  `BridgingSubmitting` with `pending_burn_tx: None` restore the active aggregate
  owner, tracking context, and exact Hedging inflight amount before their
  transfer job enters the manager. For `ConversionComplete`, the fresh source
  snapshot already reflects conversion, so recovery installs the aggregate's
  exact converted amount without debiting available again. For
  `BridgingSubmitting`, `None` means no burn transaction was durably recorded;
  it does not prove that no broadcast occurred. Reconstruction restores only the
  source reservation and does not authorize a new burn. The resumed manager must
  verify or adopt any prior broadcast before submitting another. Available
  remains the freshly observed source balance.
...
For every state outside the pre-burn Alpaca-to-Base set above, startup retains
ADR 0003's guard-only behavior: it reconstructs the global guard but does not
rebuild the active owner, tracking entry, or inventory inflight. This includes
Base-to-Alpaca states and `BridgingSubmitting` with a durably recorded
`pending_burn_tx: Some(_)`. The recorded transaction may still be pending or may
revert, so this exclusion is conservative rather than a claim that the burn
completed. A state-specific recovery path may replace guard-only recovery only
when it proves the complete inventory effect.
...
- Add separate aggregate-load-failure and multiple-source-reservation tests.
  Each must assert an error, unchanged financial state, a latched USDC guard,
  and no imbalance dispatch.
- Prove the freshness barrier rejects observations fetched before guard
  latching, queued by a prior recovery attempt, or delivered after a newer
  attempt starts; none of these stale observations may authorize resume.
