...
client crate in the `st0x.alpaca` repository and overlap with types in
`st0x.issuance`. Keeping equivalent types and arithmetic in multiple
repositories would let validation, serialization, and rounding behavior drift.
...
6. **Keep dependency direction domain-first.** The `st0x-alpaca` package in
   `st0x.alpaca` depends on the tagged `st0x-finance` contract; `st0x-finance`
   never depends on Alpaca or on either service. Liquidity and issuance retain
   their business orchestration, telemetry, persistence, and API-specific types.
...
- RAI-1165: implement the `st0x-alpaca` package in `st0x.alpaca` against the
  tagged `Symbol`, `FractionalShares`, `Usd`, and `Usdc` types.
