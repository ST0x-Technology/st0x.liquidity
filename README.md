...
**Note**: This section applies if you're migrating from a pre-CQRS/ES version of the system. New installations can skip this section.

The `migrate_to_events` binary converts legacy CRUD data to event-sourced aggregates. This is a one-time operation required before enabling the event-sourced system.

### When to Run Migration

- Migrating from legacy CRUD tables (`onchain_trades`, `trade_accumulators`, `offchain_trades`, `schwab_auth`)
...
This shows what would be migrated without actually writing events. Review the output for:
